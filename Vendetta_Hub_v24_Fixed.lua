...
-- Acquisition handlers are installed below after the core hub definitions.
...
-- Bypass handler is installed below.
...
local function StartAntiFall()
    ...
    AntiFallConnection = RS.Heartbeat:Connect(function()
        ...
        if H:GetState() == Enum.HumanoidStateType.Freefall and R.Velocity.Y < -50 then
            pcall(function()
                H:ChangeState(Enum.HumanoidStateType.RunningNoPhysics)
                local V = R.Velocity
                R.Velocity = Vector3.new(V.X,-10,V.Z)
            end)
        end
    end)
end
...
AntiFallButton.MouseButton1Click:Connect(function()
    ...
    if AntiFallDamage then StartAntiFall() else StopAntiFall() end
end)
...
--==================================================
--// V23 ACQUISITION / BYPASS SYSTEM
--// Kept after the original core definitions so the base GUI
--// can initialize exactly like the known-working v12 script.
--==================================================
do
    ...
    local function GetTools()
        ...
        if backpack then
            for _,v in ipairs(backpack:GetChildren()) do
                if v:IsA("Tool") then out[#out+1] = v end
            end
        end
        if char then
    ...
    local function StartDesync(seconds)
        ...
        task.delay(seconds,function()
            ...
            if DesyncConnection == connection then DesyncConnection = nil end
        end)
    end
    ...
    local function ActivatePrompt(obj)
    ...
    local function TweenExact(position, duration)
        local char = LP.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return false end
        local tween = TS:Create(root,TweenInfo.new(duration,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),{
    ...
    local KatanaPosition = Vector3.new(-251.04994201660156,821.5532836914062,-30.124601364135742)

    local function MakeBreathingSelector()
        if GUI:FindFirstChild("VendettaBreathingSelector") then
            GUI.VendettaBreathingSelector:Destroy()
        end

        local frame = Instance.new("Frame")
        frame.Name = "VendettaBreathingSelector"
        frame.Size = UDim2.fromOffset(270,340)
        frame.Position = UDim2.new(.5,-135,.5,-170)
            ...
            button.MouseButton1Click:Connect(function()
                ...
                task.spawn(function()
                    local ok,err = pcall(function()
                        local trainerPos = TrainerPoints[name]
                        if not trainerPos then error("Unknown trainer: "..tostring(name)) end

                        -- 1) Reach Katana Seller first.
                        local katana = FindNamed("Katana Seller")
                        local katanaPart = GetPart(katana)
                        if katanaPart then
                            if not TweenExact(katanaPart.Position,3) then error("Could not reach Katana Seller") end
                        else
                            if not TweenExact(KatanaPosition,3) then error("Could not reach Katana Seller position") end
                        end

                        -- 2) Talk to Katana Seller.
                        katana = FindNamed("Katana Seller")
                        if not ActivatePrompt(katana) then error("Katana Seller ProximityPrompt not found") end

                        -- 3) Wait 1 second, THEN desync for 2 seconds.
                        task.wait(1)
                        StartDesync(2)
                        task.wait(2)
                        StopDesync()

                        -- 4) Tween to selected trainer in exactly 1 second.
                        if not TweenExact(trainerPos,1) then error("Could not reach trainer") end

                        -- 5) Talk to the trainer. Done.
                        local trainer = FindNamed(name)
                        if not ActivatePrompt(trainer) then error(name.." ProximityPrompt not found") end
                    end)
                    StopDesync()
                    ...
                    if not ok then
                        ...
                        Notify("Vendetta Hub",name.." sequence finished.",2)
                    end
                end)
            end)
            y = y + 27
    ...
    local function RunBDA()
        if AcquisitionBusy then return end
        AcquisitionBusy = true
        task.spawn(function()
            local ok,err = pcall(function()
                local akuma = FindNamed("Old Akuma")
                local part = GetPart(akuma)
                if not part then error("Old Akuma not found") end

                -- Reach Old Akuma, then activate the first prompt ONCE.
                if not TweenExact(part.Position,3) then error("Could not reach Old Akuma") end
                if not ActivatePrompt(akuma) then error("Old Akuma ProximityPrompt not found") end
                ...
                -- Wait 2 sec, then second desync: 2 sec. No second prompt.
                task.wait(2)
                StartDesync(2)
                task.wait(2)
                StopDesync()
    ...
    -- Bypass: state cleanup + movement/injury maintenance, isolated from startup.
    ...
    local function EnableBypass()
        if BypassEnabled then
            Notify("Vendetta Hub","Bypass already enabled.",2)
            return
        end
        BypassEnabled = true
        local char = LP.Character
        if char then task.spawn(SetupBypass,char) end

        if BypassCharacterConnection then BypassCharacterConnection:Disconnect() end
        BypassCharacterConnection = LP.CharacterAdded:Connect(function(c)
            task.wait(.5)
            if BypassEnabled then SetupBypass(c) end
        end)
        ...
        task.spawn(function()
            ...
            pcall(function()
                ...
                if client and client:IsA("LocalScript") then client.Disabled = true end
            end)
        end)
    ...
    AutoBDAsButton.MouseButton1Click:Connect(RunBDA)
    BypassButton.MouseButton1Click:Connect(EnableBypass)

    print("[VENDETTA] V23 systems ready.")
end
