...
--// Added at the very end so the original startup path is untouched.
--==================================================
...
do
    ...
    local function GetAcquisitionTools()
        local Found = {}
        local Backpack = LP:FindFirstChildOfClass("Backpack")
        if Backpack then
    ...
    local function StartAcquisitionDesync(Duration)
        StopAcquisitionDesync()
        AcquisitionDesyncToken += 1
        local Token = AcquisitionDesyncToken
        ...
        AcquisitionDesyncConnection = RS.Heartbeat:Connect(function()
            ...
            for _, Tool in ipairs(GetAcquisitionTools()) do
                pcall(function()
                    Hum:EquipTool(Tool)
                    Hum:UnequipTools()
                    Hum:EquipTool(Tool)
        ...
        task.delay(Duration, function()
            if AcquisitionDesyncToken == Token then
                StopAcquisitionDesync()
            end
        end)
    end

    local function FindNamedAcquisitionTarget(Name)
        return workspace:FindFirstChild(Name, true)
    end

    local function GetAcquisitionPosition(Object)
    ...
    local function FindAcquisitionPrompt(Object)
        if not Object then return nil end
    ...
    local function TriggerAcquisitionPrompt(Name)
        local Target = FindNamedAcquisitionTarget(Name)
        if not Target then
            Notify("Vendetta Hub", Name .. " not found.", 3)
            return false
        end

        local Prompt = FindAcquisitionPrompt(Target)
        if not Prompt then
        ...
        Notify("Vendetta Hub", "fireproximityprompt is unavailable in this executor.", 3)
        return false
    end

    local function TweenAcquisitionTo(Position)
        ...
        if TrainerTween == Tween then TrainerTween = nil end
        return true
    end
    ...
    local function RunBreathingAcquisition(TrainerName)
        ...
        task.spawn(function()
            TweenAcquisitionTo(KatanaSellerPosition)
            TriggerAcquisitionPrompt("Katana Seller")
            task.wait(1)
            TweenAcquisitionTo(Position)
            StartAcquisitionDesync(3)
        end)
    end
    ...
    local function OpenBreathingSelector()
        ...
        Close.MouseButton1Click:Connect(function() Selector.Visible = false end)
    ...
    AutoBDAsButton.MouseButton1Click:Connect(function()
        task.spawn(function()
            local OldAkuma = FindNamedAcquisitionTarget("Old Akuma")
            local Position = GetAcquisitionPosition(OldAkuma)
            if not Position then
                Notify("Vendetta Hub", "Old Akuma not found.", 3)
                return
            end

            -- Reach Old Akuma, then first 2-second burst.
            TweenAcquisitionTo(Position)
            TriggerAcquisitionPrompt("Old Akuma")
            StartAcquisitionDesync(2)
            task.wait(2)
            ...
            -- Second interaction, then second 2-second burst.
            TriggerAcquisitionPrompt("Old Akuma")
            StartAcquisitionDesync(2)
        end)
    end)
