...
local TrainerPoints = {
    ["Rock Trainer"] = Vector3.new(-220.3873748779297, 825.102294921875, 1354.03173828125),
...
local function StartAntiFall()
    ...
    AntiFallConnection = RS.Heartbeat:Connect(function()
        ...
        if HumanoidNow:GetState() == Enum.HumanoidStateType.Freefall and RootPart.Velocity.Y < -50 then
            pcall(function()
...
--==================================================
--// ACQUISITION + BYPASS
--// Added after the original hub so startup remains independent.
--==================================================

do
    local AcquisitionBusy = false
    local DesyncConnection = nil
    local BypassEnabled = false
    local BypassCharacterConnection = nil
    ...
    local function StartDesync(seconds)
        StopDesync()
        ...
        connection = RS.Heartbeat:Connect(function()
            ...
            for _,tool in ipairs(GetToolList()) do
                pcall(function()
                    hum:EquipTool(tool)
                    hum:UnequipTools()
                    hum:EquipTool(tool)
        ...
        DesyncConnection = connection

        task.delay(seconds, function()
            if connection then
                connection:Disconnect()
            end
            if DesyncConnection == connection then
                DesyncConnection = nil
            end
        end)
    end
    ...
    local function TriggerPromptFromTarget(target)
    ...
    local function TriggerNamedPrompt(name)
        return TriggerPromptFromTarget(FindTarget(name))
    end

    local function MoveFast(position, seconds)
        local char = LP.Character
        ...
        local tween = TS:Create(
            root,
            TweenInfo.new(seconds, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut),
    ...
    --// BYPASS SYSTEM
    --==================================================
    local function SetupBypassCharacter(char)
        if not char then return end

        local states = char:FindFirstChild("States") or char:WaitForChild("States", 10)
        if not states then return end

        states.ChildAdded:Connect(function(child)
            local name = string.lower(child.Name)
            if name:find("stun") or name:find("ragdoll") or name:find("frozen") then
                task.defer(function()
    ...
    local function EnableBypass()
        if BypassEnabled then
            Notify("Vendetta Hub", "Bypass is already enabled.", 2)
            return
        end
        ...
        if LP.Character then
            task.spawn(SetupBypassCharacter, LP.Character)
        end

        if BypassCharacterConnection then
            BypassCharacterConnection:Disconnect()
        end
        BypassCharacterConnection = LP.CharacterAdded:Connect(function(char)
            task.wait(.5)
            if BypassEnabled then
                SetupBypassCharacter(char)
            end
        end)
        ...
        BypassLoop = task.spawn(function()
            while BypassEnabled do
                task.wait(2)
                if not BypassEnabled then break end
                pcall(ResetInjury)
                pcall(FixMovement)
            end
        end)

        -- Keep this delayed so it cannot interfere with the hub's startup.
        task.delay(1, function()
            if not BypassEnabled then return end
            pcall(function()
                local pScripts = LP:FindFirstChild("PlayerScripts")
                local client = pScripts and pScripts:FindFirstChild("client")
                if client then
        ...
        Notify("Vendetta Hub", "Bypass enabled.", 2)
    end

    --==================================================
    --// AUTO GET ALL BREATHINGS
    --// 1. Reach Katana Seller
    --// 2. Activate Katana Seller prompt
    --// 3. Wait 1 second
    --// 4. Desync for exactly 2 seconds, then stop
    --// 5. Tween to selected trainer in exactly 1 second
    --// 6. Activate trainer prompt and finish
    --==================================================
    ...
    local function RunBreathingAcquisition(trainerName)
        if AcquisitionBusy then
            Notify("Vendetta Hub", "An acquisition is already running.", 2)
            return
        end

        local trainerPos = TrainerPoints[trainerName]
        if not trainerPos then
            Notify("Vendetta Hub", "Unknown trainer: " .. tostring(trainerName), 3)
            return
        end
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                -- Reach Katana Seller first. Do not start desync before arrival.
                MoveFast(KatanaSellerPosition, 3)

                -- Talk to Katana Seller only after arrival.
                local katanaTarget = FindTarget("Katana Seller")
                local katanaPrompted = TriggerPromptFromTarget(katanaTarget)
                if not katanaPrompted then
                    Notify("Vendetta Hub", "Katana Seller ProximityPrompt not found.", 3)
                    return
                end

                -- Required one-second delay after talking.
                task.wait(1)

                -- Required two-second desync, then fully stop it.
                StartDesync(2)
                task.wait(2)
                StopDesync()

                -- Only after desync has stopped, tween to the selected trainer for 1 second.
                MoveFast(trainerPos, 1)

                -- Talk to the selected trainer. This is the final step.
                local trainerTarget = FindTarget(trainerName)
                if not TriggerPromptFromTarget(trainerTarget) then
                    Notify("Vendetta Hub", trainerName .. " ProximityPrompt not found.", 3)
                    return
                end

                Notify("Vendetta Hub", trainerName .. " finished.", 2)
            end)

            StopDesync()
            AcquisitionBusy = false
            if not ok then
                warn("[VENDETTA] Breathing error: " .. tostring(err))
                Notify("Vendetta Hub", "Breathing error: " .. tostring(err), 4)
            end
        end)
    end

    local function MakeBreathingSelector()
        local existing = GUI:FindFirstChild("BreathingSelector")
        if existing then existing:Destroy() end

        local frame = Instance.new("Frame")
        frame.Name = "BreathingSelector"
        frame.Size = UDim2.fromOffset(280, 350)
        frame.Position = UDim2.new(.5, -140, .5, -175)
        ...
        scroll.CanvasSize = UDim2.fromOffset(0, #TrainerOrder * 32)
        scroll.ZIndex = 201
        scroll.Parent = frame
        ...
        for _,name in ipairs(TrainerOrder) do
            ...
            b.MouseButton1Click:Connect(function()
                if AcquisitionBusy then return end
                frame:Destroy()
                RunBreathingAcquisition(name)
            end)
            y = y + 32
        end
    ...
    --==================================================
    --// AUTO GET ALL BDAs
    --// Reach Old Akuma -> FIRST prompt ONLY -> desync 2s
    --// stop -> wait 2s -> desync 2s -> stop.
    --// No second prompt.
    --==================================================
    local function RunBDAAcquisition()
        if AcquisitionBusy then
            Notify("Vendetta Hub", "An acquisition is already running.", 2)
            return
        end
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                local akumaTarget = FindTarget("Old Akuma")
                local akumaPart = FindPart(akumaTarget)
                if not akumaPart then
                    Notify("Vendetta Hub", "Old Akuma was not found.", 3)
                    return
                end

                -- Reach Old Akuma before doing anything else.
                MoveFast(akumaPart.Position, 3)

                -- FIRST and ONLY prompt activation.
                if not TriggerPromptFromTarget(akumaTarget) then
                    Notify("Vendetta Hub", "Old Akuma ProximityPrompt not found.", 3)
                    return
                end

                -- First desync burst: 2 seconds.
                StartDesync(2)
                task.wait(2)
                StopDesync()

                -- Wait 2 seconds with desync fully stopped.
                task.wait(2)

                -- Second desync burst: 2 seconds, no prompt this time.
                StartDesync(2)
                task.wait(2)
                StopDesync()

                Notify("Vendetta Hub", "BDA sequence finished.", 2)
            end)

            StopDesync()
            AcquisitionBusy = false
            if not ok then
                warn("[VENDETTA] BDA error: " .. tostring(err))
                Notify("Vendetta Hub", "BDA error: " .. tostring(err), 4)
            end
        end)
    end

    AutoBreathingsButton.MouseButton1Click:Connect(MakeBreathingSelector)
    AutoBDAsButton.MouseButton1Click:Connect(RunBDAAcquisition)
    BypassButton.MouseButton1Click:Connect(EnableBypass)

    print("[VENDETTA] Acquisition + bypass loaded.")
end
