...
-- Non-blocking startup for executor environments.
...
local function FindFlute()
    ...
    return ChatNPCs and ChatNPCs:FindFirstChild("Flute") or nil
end

local function FindEarrings()
    local Map = workspace:FindFirstChild("map")
    return Map and Map:FindFirstChild("Barrel") or nil
end
...
FluteTween.MouseButton1Click:Connect(function()
    ...
    if Tween then
        Notify("Vendetta Hub", "Tweening to Flute...", 2)
    end
end)
...
-- Direct item lookup is used; no chunk/terrain scan is performed.
...
local function RunBreathingAcquisition(TrainerName)
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            TweenAcquisition(KatanaSellerPosition)
            task.wait(.25)
            ...
            TweenAcquisition(TrainerPosition)
            task.wait(.25)
...
BDANote.Text = "BDA acquisition: Old Akuma • automatic prompt + Yes/Accept/OK dialog handling"
...
local function AutoAcceptBDAButtons(Duration)
            ...
            for _, Obj in ipairs(PlayerGui:GetDescendants()) do
                if Obj:IsA("TextButton") or Obj:IsA("ImageButton") then
                    ...
                    if Obj:IsA("TextButton") then
                        Text = tostring(Obj.Text):lower():gsub("%s+", "")
                    end
...
local function GetOldAkumaBDA()
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            TweenAcquisition(OldAkumaPosition)
            task.wait(1)

            local NPC = workspace:FindFirstChild("Old Akuma", true)
            if not NPC then error("Old Akuma was not found") end
            local Prompt = NPC:FindFirstChildWhichIsA("ProximityPrompt", true)
            if not Prompt then error("No ProximityPrompt found inside Old Akuma") end

            pcall(function() fireproximityprompt(Prompt) end)
            task.spawn(function() AutoAcceptBDAButtons(5) end)
            task.wait(0.5)
            pcall(function() fireproximityprompt(Prompt) end)
            task.wait(0.5)
            Notify("Vendetta Hub", "Old Akuma BDA sequence finished.", 2)
        end)
        AcquisitionRunning = false
        if not Ok then
...
OldAkumaButton.Activated:Connect(GetOldAkumaBDA)
