...
-- Breathing-only trainer lookup: workspace.chatnpcs
local function FindBreathingTrainerPrompt(TrainerName)
    local Container = workspace:FindFirstChild("chatnpcs")
    if not Container then return nil end

    local Query = string.lower(TrainerName:gsub("%s+[Tt]rainer$", ""))
    local Partial
    ...
    for _,Object in ipairs(Container:GetDescendants()) do
        if Object:IsA("Model") then
            ...
            local Prompt = Object:FindFirstChildWhichIsA("ProximityPrompt", true)
            if Prompt then
                if ObjectName == Query or ObjectName == Query .. " trainer" then
                    return Prompt
                elseif not Partial and ObjectName:find(Query, 1, true) then
                    Partial = Prompt
                end
    ...
    for _,Name in ipairs(TrainerOrder) do
        ...
        TrainerButton.Activated:Connect(function()
            ...
            task.spawn(function()
                ...
                task.wait(1)
                if TweenAndWait(Target) then
                    task.wait(0.1)
                    local TrainerPrompt = FindBreathingTrainerPrompt(Name)
                    if TrainerPrompt then
                        TriggerPrompt(TrainerPrompt)
                    else
                        Notify("Vendetta Hub", Name .. " prompt not found in workspace.chatnpcs.", 3)
                    end
                end
