...
for Name,PageName in pairs({
    Main = "Main", Mobility = "Mobility", Farm = "Farm", Server = "Server",
    Tween = "Tween", OP = "OP", Credits = "Credits",
    Info = "Information", Settings = "Settings"
}) do
...
--==================================================
--// NOTIFICATIONS (declared before every use site)
--==================================================

local StarterGui = game:GetService("StarterGui")

local function Notify(NotifyTitle,Text,Duration)
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = NotifyTitle, Text = Text, Duration = Duration or 3
        })
    end)
end
...
Label(MainPage,"MISC",960)
local FPSBoostButton = Button(MainPage,"FPS Booster   •   OFF",992)
local FullBrightButton = Button(MainPage,"Full Bright   •   OFF",1036)

local FriendTitle = Label(MainPage,"MANUAL FRIEND IGNORE",1080)
local FriendBox = Instance.new("TextBox",MainPage)
FriendBox.Size = UDim2.new(1,-116,0,38)
FriendBox.Position = UDim2.fromOffset(6,1114)
...
local AddFriendButton = Button(MainPage,"ADD",1120)
AddFriendButton.Size = UDim2.fromOffset(48,38)
AddFriendButton.Position = UDim2.new(1,-104,0,1114)
local RemoveFriendButton = Button(MainPage,"REMOVE",1172)
RemoveFriendButton.Size = UDim2.fromOffset(98,38)
RemoveFriendButton.Position = UDim2.fromOffset(6,1160)

local FriendListLabel = Label(MainPage,"Ignored: none",1214)
FriendListLabel.TextWrapped = true
FriendListLabel.Size = UDim2.new(1,-12,0,60)

--==================================================
--// KILL SELF + ANTI FALL DAMAGE
--==================================================

local KillSelfButton = Button(MainPage,"Kill Self",1280)
KillSelfButton.MouseButton1Click:Connect(function()
    local hum = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum then hum.Health = 0 end
end)

local AntiFallButton = Button(MainPage,"Anti Fall Dmg   •   ON",1324)

local function SetAntiFall(State)
    AntiFallEnabled = State
    AntiFallButton.Text = "Anti Fall Dmg   •   "..(State and "ON" or "OFF")
    SetToggleVisual(AntiFallButton, State)
end
...
--==================================================
--// OP — BUTTONS + AUTO FLOWS
--==================================================

local BypassButton = Button(OP,"Bypass   •   OFF",8)
local GetBreathingsButton = Button(OP,"Auto Get All Breathings",52)
local GetBDAsButton = Button(OP,"Auto Get All BDAs",96)

local BreathingTrainers = {
...
local BDAList = {
    "Ravenous","Frost","Temari","Arrow Demon",
    "Swamp","Drum","Thread","Tongue",
    "Soul Eater","Blue Spider Lily",
}

local KatanaSellerPos = Vector3.new(-251.04994201660156, 821.5532836914062, -30.124601364135742)

--==================================================
--// OP — TWEEN + NPC HELPERS  (50 studs/sec)
--==================================================

local OP_TWEEN_SPEED = 50 -- studs per second

local function TweenPlayerTo(Pos, Speed)
    local char = LP.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local dist = (Pos - root.Position).Magnitude
    local spd  = Speed or OP_TWEEN_SPEED
    local dur  = math.max(0.05, dist / spd)
    local t = TS:Create(
        root,
...
local function FindNearestNPC(Pos, Radius)
    local nearest, best = nil, Radius or math.huge
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj ~= LP.Character
...
local function InteractNPC(npc)
    ...
    local remote = events and events:FindFirstChild("remote")
    if remote then
        pcall(function() remote:FireServer("Talk") end)
        pcall(function() remote:FireServer("Interact") end)
        pcall(function() remote:FireServer("Dialogue") end)
        pcall(function() remote:FireServer("TalkTo", npc) end)
    end
end

--==================================================
--// INVENTORY DESYNC (internal)
--==================================================
...
local function GetAllTools()
    ...
    if bp then
        for _, v in ipairs(bp:GetChildren()) do
            if v:IsA("Tool") then table.insert(found, v) end
        end
    end
    local char = LP.Character
    if char then
...
function StartDesync()
    ...
    DesyncConn = RS.Heartbeat:Connect(function()
        ...
        for _, t in ipairs(tools) do
            pcall(function()
                hum:EquipTool(t); hum:UnequipTools()
                hum:EquipTool(t); hum:UnequipTools()
                hum:EquipTool(t); hum:UnequipTools()
                t.Parent = nil
                t.Parent = LP:FindFirstChildOfClass("Backpack")
            end)
        end
    end)
end
...
--==================================================
--// BYPASS — STATE / INJURY / MOVEMENT / CLIENT
--==================================================

local BypassLoop = nil
local BypassStateConns = {}

local function Bypass_SetupStates(char)
    if not char then return end
    local states = char:WaitForChild("States", 10)
    if not states then return end
    local conn = states.ChildAdded:Connect(function(child)
        local n = child.Name:lower()
        if n:find("stun") or n:find("ragdoll") or n:find("frozen") then
            task.defer(function()
                if child and child.Parent then child:Destroy() end
            end)
        end
    end)
    table.insert(BypassStateConns, conn)
end

local function Bypass_ResetInjury()
    local injury = LP:FindFirstChild("Injury")
...
local function Bypass_FixMovement()
    local char = LP.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
...
local function Bypass_DisableClient()
    local pScripts = LP:FindFirstChild("PlayerScripts")
    if not pScripts then return end
    local c = pScripts:FindFirstChild("client")
    if c then c.Disabled = true end
end

local function Bypass_Start()
    -- Attach to current character + future ones
    if LP.Character then Bypass_SetupStates(LP.Character) end

    -- Loop: reset injury + fix movement every 2s
    BypassLoop = task.spawn(function()
        while BypassToggled do
            task.wait(2)
            pcall(Bypass_ResetInjury)
            pcall(Bypass_FixMovement)
        end
    end)

    task.wait(1)
    Bypass_DisableClient()
end

local function Bypass_Stop()
    BypassToggled = false
    for _, c in ipairs(BypassStateConns) do
        pcall(function() c:Disconnect() end)
    end
    BypassStateConns = {}
    if BypassLoop then
        pcall(function() task.cancel(BypassLoop) end)
        BypassLoop = nil
    end
end
...
BypassButton.MouseButton1Click:Connect(function()
    ...
    BypassButton.Text = "Bypass   •   "..(BypassToggled and "ON" or "OFF")
    SetToggleVisual(BypassButton, BypassToggled)
    if BypassToggled then
        Bypass_Start()
        Notify("Vendetta Hub","Bypass ON — States cleared, Injury reset, Client disabled.",3)
    else
        Bypass_Stop()
        Notify("Vendetta Hub","Bypass OFF.",2)
    end
end)
...
local function HasKatana()
    local char = LP.Character
    local bp   = LP:FindFirstChildOfClass("Backpack")
    if char and char:FindFirstChild("Katana") then return true end
    if bp   and bp:FindFirstChild("Katana")   then return true end
    return false
end

local function RunBreathingFlow(TrainerName)
    task.spawn(function()
        Notify("Vendetta Hub","Getting "..TrainerName.."...",2)

        if not HasKatana() then
            TweenPlayerTo(KatanaSellerPos)
            task.wait(0.6)
            local seller = FindNearestNPC(KatanaSellerPos, 50)
            InteractNPC(seller); task.wait(2)
            InteractNPC(seller); task.wait(1.5)
        end

        local tPos = TrainerPoints[TrainerName]
        if tPos then
            TweenPlayerTo(tPos)
            task.wait(0.6)
            local trainer = FindNearestNPC(tPos, 50)
            InteractNPC(trainer); task.wait(2)
            InteractNPC(trainer); task.wait(1)
        end

        Notify("Vendetta Hub","Done: "..TrainerName,2)
    end)
end

local function RunBDAFlow(BDAName)
    task.spawn(function()
        Notify("Vendetta Hub","Getting "..BDAName.."...",2)

        local akuma
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
                if obj.Name:lower():find("akuma") then akuma = obj; break end
            end
        end

        if not akuma then
            Notify("Vendetta Hub","Old Akuma not found.",3)
            return
        end

        local root = akuma:FindFirstChild("HumanoidRootPart") or akuma.PrimaryPart
        if root then
            TweenPlayerTo(root.Position + Vector3.new(0,3,4))
        end
        task.wait(0.6)
        InteractNPC(akuma); task.wait(2)
        InteractNPC(akuma); task.wait(1)
        ...
        Notify("Vendetta Hub","Done: "..BDAName,2)
    end)
end
...
local function MakeDraggable(Object)
    local Dragging, Start, StartPosition = false, nil, nil
...
--==================================================
--// DRAGGABLE + MINIMIZABLE POPUP BUILDER
--==================================================

local function BuildAutoGUI(PopupTitle, Items, OnSelect, DefaultPos, ItemHeight)
    ItemHeight = ItemHeight or 34

    local Holder = Instance.new("Frame")
    Holder.Size = UDim2.fromOffset(260, 60 + (#Items * (ItemHeight + 4)) + 12)
    Holder.Position = DefaultPos or UDim2.new(0.5,-130,0.5,-170)
    ...
    top.BackgroundColor3 = Color3.fromRGB(20,8,8)
    ...
    minBtn.TextSize = 16
    minBtn.TextColor3 = Color3.fromRGB(235,45,45)
    minBtn.BackgroundColor3 = Color3.fromRGB(35,12,12)
    ...
    local body = Instance.new("ScrollingFrame", Holder)
    body.Size = UDim2.new(1,-12,1,-48)
    body.Position = UDim2.fromOffset(6,42)
    body.BackgroundTransparency = 1
    body.BorderSizePixel = 0
    body.ScrollBarThickness = 3
    body.ScrollBarImageColor3 = Color3.fromRGB(235,45,45)
    body.CanvasSize = UDim2.new(0,0,0,#Items * (ItemHeight + 4))
    body.ZIndex = 51

    for i, name in ipairs(Items) do
        local b = Instance.new("TextButton", body)
        b.Size = UDim2.new(1,-6,0,ItemHeight)
        b.Position = UDim2.fromOffset(3,(i-1)*(ItemHeight + 4))
        b.Text = "Get "..name
        b.TextSize = 12
        b.Font = Enum.Font.GothamSemibold
        b.TextColor3 = Color3.fromRGB(240,240,245)
        ...
        b.MouseEnter:Connect(function()
            TS:Create(b,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(35,18,18)}):Play()
        end)
        b.MouseLeave:Connect(function()
            TS:Create(b,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(19,20,27)}):Play()
        end)
        b.MouseButton1Click:Connect(function()
            pcall(function() OnSelect(name) end)
        end)
    end
    ...
    Mini.Size = UDim2.fromOffset(120,40)
    Mini.Position = Holder.Position
    Mini.Text = PopupTitle
    Mini.TextSize = 11
    ...
    miniStroke.Color = Color3.fromRGB(235,45,45)
    ...
    return {
        ...
        Mini = Mini,
        Show = function()
            if minimized then Mini.Visible = true else Holder.Visible = true end
        end,
        Hide = function()
            Holder.Visible = false
            Mini.Visible = false
...
local BreathingPopup = BuildAutoGUI(
    "GET BREATHING",
    BreathingTrainers,
    function(TrainerName) RunBreathingFlow(TrainerName) end,
    UDim2.new(0.15, 0, 0.20, 0),
    34
)

local BDAPopup = BuildAutoGUI(
    "GET BDA",
    BDAList,
    function(BDAName) RunBDAFlow(BDAName) end,
    UDim2.new(0.60, 0, 0.20, 0),
    34
)

GetBreathingsButton.MouseButton1Click:Connect(function()
    BreathingPopup.Show()
end)

GetBDAsButton.MouseButton1Click:Connect(function()
    BDAPopup.Show()
end)
...
local function MakeDraggableMain(Object)
...
local function CreateHealthESP(P)
    ...
    task.spawn(function()
        while Billboard.Parent and HealthESP do
            ...
            Distance.Text =
                math.floor((MyRoot.Position - TheirRoot.Position).Magnitude) .. " studs"
            task.wait(.12)
        end
    end)
end
...
local function RefreshPlayers()
    for _,P in ipairs(Players:GetPlayers()) do
        if P ~= LP then
            if HealthESP then CreateHealthESP(P) end
            if NameDistanceESP then CreateNameDistanceESP(P) end
        end
    end
end
...
HealthButton.MouseButton1Click:Connect(function()
    ...
    HealthButton.Text = "Health ESP   •   " .. (HealthESP and "ON" or "OFF")
...
NameDistanceButton.MouseButton1Click:Connect(function()
    ...
    NameDistanceButton.Text = "Name + Distance ESP   •   "..(NameDistanceESP and "ON" or "OFF")
...
local function FindFlute()
    ...
    if Flute:IsA("BasePart") or Flute:IsA("Model") then return Flute end
    return nil
end
...
local function FindEarrings()
    ...
    for _,Object in ipairs(Map:GetChildren()) do
        if Object:IsA("Model") and Object.Name == "Barrel" then
            return Object
        end
    end
    return nil
...
FluteESPButton.MouseButton1Click:Connect(function()
    ...
    FluteESPButton.Text = "Flute ESP   •   " .. (FluteESP and "ON" or "OFF")
    SetToggleVisual(FluteESPButton, FluteESP)
    if FluteESP then
        local Part = FindFlute()
        if Part then
            CreateObjectESP(Part, function() return "♫  FLUTE" end, "FLUTE")
        end
    else
        ClearESP("FLUTE")
    end
...
EarringsESPButton.MouseButton1Click:Connect(function()
    ...
    EarringsESPButton.Text = "Earrings ESP   •   " .. (EarringsESP and "ON" or "OFF")
    SetToggleVisual(EarringsESPButton, EarringsESP)
    if EarringsESP then
        local Part = FindEarrings()
        if Part then
            CreateObjectESP(Part, function() return "◈  EARRINGS" end, "EARRINGS")
        end
    else
        ClearESP("EARRINGS")
    end
...
local function TweenToPosition(Position)
    ...
    TrainerTween:Play()
    TrainerTween.Completed:Once(function() TrainerTween = nil end)
end
...
local function GetIceShard() return workspace:FindFirstChild("ActiveIceShard") end
local function GetIceShardPart(Shard)
    if not Shard then return nil end
    if Shard:IsA("BasePart") or Shard:IsA("Model") then return Shard end
    return nil
end
...
local function TweenTo(Target)
    ...
    ItemTween:Play()
    ItemTween.Completed:Once(function() ItemTween = nil end)
    return ItemTween
end
...
ArmButton.MouseButton1Click:Connect(function()
    NoArm = not NoArm
    ArmButton.Text = "No Arm   •   " .. (NoArm and "ON" or "OFF")
...
local function TryAutoBlock()
    ...
    BlockingNow = true
    pcall(function() BlockRemote:FireServer("blockstart") end)
    task.delay(0.55,function()
        pcall(function() BlockRemote:FireServer("blockend") end)
        BlockingNow = false
    end)
end
            ...
            for _,Enemy in ipairs(Players:GetPlayers()) do
                if Enemy ~= LP and not ShouldIgnore(Enemy) and Enemy.Character then
                    ...
                    if Root and (Root.Position - HRP.Position).Magnitude <= 50 then
                        if Windup then TryAutoBlock() end
                        if CDS then
                            ...
                            for MoveName in pairs(MoveNames) do
                                ...
                                if Active and not Previous then TryAutoBlock() end
...
local function ActivateStrongAttack()
    ...
    return pcall(function() Event:FireServer("StrongAttack") end)
end
                    ...
                    if Root and CDS and (Root.Position-Me.Position).Magnitude <= 10 then
                        ...
                        if IsBlocking and not LastBlock[Enemy] then
                            if AutoStrongAttack then
                                task.spawn(function() ActivateStrongAttack() end)
                            end
...
local function UpdateHitbox(P)
    ...
    local C = P.Character
    if not C then RemoveHitbox(P) return end
    local H = HitboxObjects[P]
...
task.spawn(function()
    while GUI.Parent do
        if AutoBreathing then
            pcall(function() FeatureRemote:FireServer("manacharges") end)
        end
        task.wait(2)
    end
end)
...
KatanaButton.MouseButton1Click:Connect(function()
    ...
    if KatanaHidden then
        pcall(function() FeatureRemote:FireServer("KatanaUneq") end)
    end
end)
...
local function HoldButton(ButtonObject,Direction)
    ...
    ButtonObject.InputEnded:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.Touch
        or Input.UserInputType == Enum.UserInputType.MouseButton1 then
            if Vertical == Direction then Vertical = 0 end
        end
    end)
end
...
local function SetSpeed(Value)
    FlySpeed = math.clamp(tonumber(Value) or FlySpeed, 1, 500)
    SpeedBox.Text = tostring(math.floor(FlySpeed))
    SpeedDisplay.Text = tostring(math.floor(FlySpeed))
end

SpeedBox.FocusLost:Connect(function() SetSpeed(SpeedBox.Text) end)
Plus.MouseButton1Click:Connect(function() SetSpeed(FlySpeed + 10) end)
Minus.MouseButton1Click:Connect(function() SetSpeed(FlySpeed - 10) end)
...
local function FindPlayer(Text)
    ...
    Text = Text:lower()
    for _,P in ipairs(Players:GetPlayers()) do
        if P.Name:lower() == Text then return P end
    end
    for _,P in ipairs(Players:GetPlayers()) do
...
FarmButton.MouseButton1Click:Connect(function()
    if Farm then StopFarm() return end
...
local function FindNPC(Text)
    ...
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("Model") and Object ~= LP.Character then
            ...
            if H and Object.Name:lower() == Query then return Object end
        end
    end
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("Model") and Object ~= LP.Character then
            local H = Object:FindFirstChildOfClass("Humanoid")
            if H and Object.Name:lower():sub(1,#Query) == Query then return Object end
        end
    end
    return nil
end
...
local function StartNPCFarm(Target)
    ...
    task.spawn(function()
        ...
        while NPCFarm and GUI.Parent and Remote do
            if not Target.Parent or TargetHum.Health <= 0 or not HRP then break end
            ...
            if not Root then
                ...
                if not NPCFarm or TargetHum.Health <= 0 then break end
                ...
                if NPCFarm and TargetHum.Health > 0 then
...
NPCFarmButton.MouseButton1Click:Connect(function()
    if NPCFarm then StopNPCFarm() return end
...
local GourdNames = {
    "Small Gourd","Medium Gourd","Big Gourd","Massive Gourd",
}
    ...
    task.spawn(function()
        ...
        while AutoGourds and GUI.Parent and Remote do
            ...
            for _,GourdName in ipairs(GourdNames) do
                ...
                if Gourd then
                    UsedAny = true
                    pcall(function() Remote:FireServer("manacharges") end)
                    task.wait(2)
                    if not AutoGourds then break end
                    pcall(function() Remote:FireServer(GourdName, nil, nil) end)
                    task.wait(5)
                    if FindOwnedGourd(GourdName) then break end
                end
            end
...
AutoGourdsButton.MouseButton1Click:Connect(function()
    ...
    if AutoGourds then StartAutoGourds() end
end)
    ...
    task.spawn(function()
        for Index,Code in ipairs(RedeemCodes) do
            ...
            if Index < #RedeemCodes then
                if Index % 5 == 0 then task.wait(1) else task.wait(0.5) end
            end
        end
        RedeemCodesRunning = false
        RedeemCodesButton.Text = "Redeem All Codes"
        SetToggleVisual(RedeemCodesButton, false)
        Notify("Vendetta Hub", "Finished sending all "..#RedeemCodes.." codes.", 3)
    end)
end)
...
ServerHopButton.MouseButton1Click:Connect(function()
    ...
    local Data
    local DecodeSuccess = pcall(function() Data = HttpService:JSONDecode(Result.Body) end)
...
Players.PlayerRemoving:Connect(function(P)
    ClearESP("HEALTH_" .. P.UserId)
    ClearESP("NAME_" .. P.UserId)
    if FarmTarget == P then StopFarm() end
end)
...
LP.CharacterAdded:Connect(function(C)
    ...
    Humanoid = C:WaitForChild("Humanoid")
    task.wait(.5)
    if NoArm then SetArms(true) end
    if Flying then task.defer(function() SetFlying(true) end) end
end)
...
task.spawn(function()
    while GUI.Parent do
        ...
        if HealthESP or NameDistanceESP then RefreshPlayers() end
        task.wait(1)
    end
end)
