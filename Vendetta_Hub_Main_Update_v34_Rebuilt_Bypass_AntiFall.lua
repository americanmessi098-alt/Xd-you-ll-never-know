...
--==================================================
--// BYPASS + ANTI FALL DAMAGE
--// Rebuilt from the working v33 structure.
--// Functionality preserved:
--//   • Removes matching combat/movement state objects
--//   • Resets Injury numeric values
--//   • Restores basic Humanoid movement states
--//   • Continuously activates the mobile Run button
--//   • Applies the same falling-velocity mitigation
--//==================================================

local BypassEnabled = false
local AntiFallDamageEnabled = false

local AntiFallDamageHeartbeatConn = nil
local BypassHeartbeatConn = nil
local BypassStateConn = nil
local BypassCharacterConn = nil
local BypassRunButtonWatcher = nil
local BypassRunBtn = nil
...
local function FindBypassRunButton()
...
local function StartAntiFallDamage()
    ...
    if AntiFallDamageHeartbeatConn then
        pcall(function()
            AntiFallDamageHeartbeatConn:Disconnect()
        end)
        AntiFallDamageHeartbeatConn = nil
    end

    AntiFallDamageHeartbeatConn = RS.Heartbeat:Connect(function()
        if not AntiFallDamageEnabled then
            return
        end
        ...
        local Success, VelocityY = pcall(function()
            return RootPart.AssemblyLinearVelocity.Y
        end)

        if not Success or VelocityY >= -50 then
            return
        end
...
local function StopAntiFallDamage()
...
local function SetupBypassStates(Char)
    if BypassStateConn then
        pcall(function()
            BypassStateConn:Disconnect()
        end)
        BypassStateConn = nil
    end
    ...
    if not States then
        local Success, Result = pcall(function()
            return Char:WaitForChild("States", 10)
        end)
    ...
    local function CheckState(Child)
        if not BypassEnabled or not Child then
            return
        end
        ...
        local BlockedState =
            string.find(Name, "stun", 1, true)
            or string.find(Name, "ragdoll", 1, true)
            or string.find(Name, "frozen", 1, true)
            or string.find(Name, "slow", 1, true)
            or string.find(Name, "block", 1, true)
            or string.find(Name, "restrain", 1, true)
            or string.find(Name, "bind", 1, true)
    ...
    for _, Child in ipairs(States:GetChildren()) do
        CheckState(Child)
    end

    BypassStateConn = States.ChildAdded:Connect(CheckState)
end

local function ResetBypassInjury()
    if not BypassEnabled then
        return
    end
    ...
    for _, Object in ipairs(Injury:GetDescendants()) do
...
local function FixBypassMovement()
    if not BypassEnabled then
        return
    end
...
local function StartBypass()
    ...
    BypassRunBtn = FindBypassRunButton()

    if BypassRunButtonWatcher then
        task.cancel(BypassRunButtonWatcher)
        BypassRunButtonWatcher = nil
    end

    BypassRunButtonWatcher = task.spawn(function()
        while BypassEnabled do
            if not BypassRunBtn or not BypassRunBtn.Parent then
                BypassRunBtn = FindBypassRunButton()
            end
    ...
    if BypassHeartbeatConn then
        pcall(function()
            BypassHeartbeatConn:Disconnect()
        end)
        BypassHeartbeatConn = nil
    end

    BypassHeartbeatConn = RS.Heartbeat:Connect(function()
        if not BypassEnabled then
            return
        end
        ...
        pcall(function()
            BypassRunBtn:Activate()
        end)
    end)

    if BypassCharacterConn then
        pcall(function()
            BypassCharacterConn:Disconnect()
        end)
        BypassCharacterConn = nil
    end

    BypassCharacterConn = LP.CharacterAdded:Connect(function(Char)
        task.wait(0.5)
        ...
        SetupBypassStates(Char)
        FixBypassMovement()
    end)

    if LP.Character then
        task.spawn(function()
            SetupBypassStates(LP.Character)
            FixBypassMovement()
        end)
    end
    ...
    task.spawn(function()
        while BypassEnabled do
            ...
            if BypassEnabled then
                pcall(ResetBypassInjury)
                pcall(FixBypassMovement)
            end
        end
    end)

    BypassButton.Text = "Bypass   •   ON"
    SetToggleVisual(BypassButton, true)
    Notify("Vendetta Hub", "Bypass enabled.", 3)
end
...
local function StopBypass()
    ...
    SetToggleVisual(BypassButton, false)
    Notify("Vendetta Hub", "Bypass disabled.", 3)
end
