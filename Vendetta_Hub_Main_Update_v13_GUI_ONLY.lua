...
-- Acquisition GUI handlers are attached later, after MakeDraggable is defined.
...
--==================================================
--// AUTO GET WINDOWS — GUI ONLY
--// Kept separate from the v12 startup code.
--==================================================
local function CreateAcquisitionGUI(Name, Title, Height)
    local Frame = Instance.new("Frame")
    Frame.Name = Name
    Frame.Size = UDim2.fromOffset(310, Height)
    Frame.Position = UDim2.new(0.5, -155, 0.5, -(Height / 2))
    ...
    Header.Position = UDim2.fromOffset(14, 5)
...
local function AcquisitionOption(Parent, Text)
    local B = Instance.new("TextButton", Parent)
...
local BreathingWindow, BreathingBody = CreateAcquisitionGUI(
    "VendettaBreathingWindow", "AUTO GET BREATHINGS", 360
)

for _, TrainerName in ipairs(TrainerOrder) do
    AcquisitionOption(BreathingBody, TrainerName)
end

local BDAWindow, BDABody = CreateAcquisitionGUI(
    "VendettaBDAWindow", "AUTO GET BDAs", 240
)

AcquisitionOption(BDABody, "Old Akuma BDA")
