...
--==================================================
--// ACQUISITION + BYPASS + ANTI FALL DAMAGE V2
--// Lazy / fault-tolerant addon: never blocks the main GUI on a remote.
--==================================================
do
    ...
    local function GetRemote()
        ...
        return events and events:FindFirstChild("remote")
    end

    local function DisconnectDesync()
    ...
    -- Desync routine is deliberately self-contained and always has an explicit stop.
    ...
    local function TriggerPrompt(name)
        ...
        if not prompt then
            Notify("Vendetta Hub", "ProximityPrompt not found: "..tostring(name), 3)
            return false
        end
    ...
    local function MoveFast(position, seconds)
        ...
        while not completed and root.Parent do task.wait() end
        return true
    end
    ...
    local function SetupBypass(char)
        ...
        if not states then
            states = char:WaitForChild("States", 5)
        end
    ...
    local function EnableBypass()
        ...
        BypassEnabled = true
        if LP.Character then task.spawn(SetupBypass, LP.Character) end
        ...
        BypassCharacterConnection = LP.CharacterAdded:Connect(function(char)
            task.wait(0.5)
            if BypassEnabled then SetupBypass(char) end
        end)
        task.spawn(function()
            while BypassEnabled and GUI.Parent do
                task.wait(2)
                pcall(ResetInjury)
                pcall(FixMovement)
            end
    ...
    RS.Heartbeat:Connect(function()
        ...
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if hum and root and root.AssemblyLinearVelocity.Y < -50 then
            pcall(function()
                hum:ChangeState(Enum.HumanoidStateType.RunningNoPhysics)
                local v = root.AssemblyLinearVelocity
                root.AssemblyLinearVelocity = Vector3.new(v.X, -10, v.Z)
            end)
        end
    end)

    -- Keep the existing button as the real toggle.
    ...
    local function MakeSelector()
        local old = GUI:FindFirstChild("BreathingSelector")
            ...
            b.MouseButton1Click:Connect(function()
                ...
                task.spawn(function()
                    local ok, err = pcall(function()
                        local katana = FindPart(FindTarget("Katana Seller"))
                        local trainerPos = TrainerPoints[name]
                        if not katana then
                            Notify("Vendetta Hub", "Katana Seller not found.", 3)
                            return
                        end
                        if not trainerPos then return end

                        -- EXACT ORDER:
                        -- reach Katana -> prompt -> wait 1s -> desync 2s -> stop
                        -- -> tween trainer in 1s -> trainer prompt -> finish
                        if not MoveFast(katana.Position, 3) then return end
                        TriggerPrompt("Katana Seller")
                        task.wait(1)
                        StartDesync(2)
                        task.wait(2)
                        DisconnectDesync()
                        if not MoveFast(trainerPos, 1) then return end
                        TriggerPrompt(name)
                    end)
    ...
    AutoBDAsButton.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                ...
                if not MoveFast(akuma.Position, 3) then return end

                -- Only the first prompt.
                TriggerPrompt("Old Akuma")

                -- Desync 2s -> stop -> wait 2s -> desync 2s -> stop.
                StartDesync(2)
                task.wait(2)
                DisconnectDesync()
                task.wait(2)
    ...
    print("[VENDETTA] Acquisition / bypass / anti-fall V2 loaded.")
end
