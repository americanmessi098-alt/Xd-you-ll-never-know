...
--==================================================
--==================================================
--// AUTO ACQUISITION WINDOWS (SAFE INIT)
--==================================================

local AcquisitionInitOK, AcquisitionInitError = pcall(function()
--// AUTO ACQUISITION WINDOWS
--// Breathing acquisition uses a separate draggable/minimizable window.
--// Movement to acquisition targets is always a 2-second tween.
--==================================================
...
AcquisitionGui.DisplayOrder = 1000000
...
local function MakeAcquisitionWindow(Name, TitleText, Height)
    ...
    -- Drag header only, so the buttons remain easy to press on mobile.
...
local BreathingWindow, BreathingBody = MakeAcquisitionWindow("BreathingWindow", "AUTO GET BREATHINGS", 360)
local BDAWindow, BDABody = MakeAcquisitionWindow("BDAWindow", "AUTO GET BDAs", 240)
...
local function TweenAcquisition(Position)
    local Root = GetRoot(LP)
    if not Root then return false end
    local Distance = (Position - Root.Position).Magnitude
    local Goal = CFrame.new(Position + Vector3.new(0,3,0))
    local Tween = TS:Create(Root, TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {CFrame = Goal})
    Tween:Play()
...
-- Inventory desync is intentionally not used in acquisition flows.

local BreathingMode = "Tween"
local BDAMode = "Tween"

local ModeLabel = Instance.new("TextLabel", BreathingBody)
ModeLabel.Size = UDim2.new(1,-4,0,24)
ModeLabel.BackgroundTransparency = 1
ModeLabel.Text = "Movement Mode: TWEEN"
ModeLabel.Font = Enum.Font.GothamSemibold
ModeLabel.TextSize = 11
ModeLabel.TextColor3 = Color3.fromRGB(220,220,225)

local ModeButton = AcquisitionButton(BreathingBody, "Mode: TWEEN")
ModeButton.MouseButton1Click:Connect(function()
    BreathingMode = BreathingMode == "Tween" and "Teleport" or "Tween"
    ModeButton.Text = "Mode: "..string.upper(BreathingMode)
    ModeLabel.Text = "Movement Mode: "..string.upper(BreathingMode)
end)
...
BDAModeButton.MouseButton1Click:Connect(function()
...
local function MoveAcquisition(Position, Mode)
    ...
    return TweenAcquisition(Position)
end
        ...
        if PG then
            for _, Gui in ipairs(PG:GetDescendants()) do
                if Gui:IsA("TextButton") then
                    local Text = tostring(Gui.Text):lower():gsub("%s+", "")
...
local function InteractWithNamedNPC(NPCName, Position, Duration)
    local NPC = workspace:FindFirstChild(NPCName, true)
    if not NPC then
        Notify("Vendetta Hub", NPCName.." was not found.", 3)
        return false
    end
    local Prompt = NPC:FindFirstChildWhichIsA("ProximityPrompt", true)
    if not Prompt then
        Notify("Vendetta Hub", "No ProximityPrompt found in "..NPCName..".", 3)
        return false
    end
    pcall(function() fireproximityprompt(Prompt) end)
    AutoAcceptDialogOnce(Duration or 3)
    return true
end
...
local function RunBreathingAcquisition(TrainerName)
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            if not TrainerPosition then error("Unknown trainer") end
        ...
        if not Ok then
            Notify("Vendetta Hub", "Acquisition error: "..tostring(Err), 4)
        end
    end)
end
...
BDANote.Size = UDim2.new(1,-4,0,58)
...
OldAkumaButton.MouseButton1Click:Connect(function()
    if AcquisitionRunning then
        Notify("Vendetta Hub", "An acquisition is already running.", 3)
        return
    end
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            task.wait(1)
            InteractWithNamedNPC("Old Akuma", OldAkumaPosition, 5)
            Notify("Vendetta Hub", "Old Akuma sequence finished.", 2)
        end)
        AcquisitionRunning = false
        if not Ok then
            Notify("Vendetta Hub", "BDA acquisition error: "..tostring(Err), 4)
        end
    end)
end)
...
if not AcquisitionInitOK then
    warn("[VENDETTA] Acquisition UI failed to initialize: "..tostring(AcquisitionInitError))
end
