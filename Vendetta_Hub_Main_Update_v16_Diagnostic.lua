--// VENDETTA HUB v16 - STARTUP DIAGNOSTIC
--// Built directly from the working v12 base.
local __V_OK, __V_ERR = xpcall(function()
...
end, function(__V_ERRMSG)
	local __V_TRACE = debug and debug.traceback and debug.traceback(__V_ERRMSG, 2) or tostring(__V_ERRMSG)
	warn("[VENDETTA DIAGNOSTIC] STARTUP/ RUNTIME ERROR:")
	warn(tostring(__V_TRACE))
	return __V_TRACE
end)

if __V_OK then
	print("[VENDETTA DIAGNOSTIC] Vendetta Hub finished startup without a synchronous error.")
else
	warn("[VENDETTA DIAGNOSTIC] Vendetta Hub DID NOT START.")
	warn("[VENDETTA DIAGNOSTIC] The traceback above contains the real script line. Delta may also show its own wrapper as Line 1.")
end
