...
--==================================================
--// WHITELIST + EXTERNAL INITIALIZER
--==================================================

local Whitelist = {
    ["bigjeff12423"] = 4417193505,
}

local function IsWhitelisted()
    return Whitelist[LP.Name] == LP.UserId
end

if not IsWhitelisted() then
    Notify("Vendetta Hub", "You are not whitelisted.", 5)
    return
end

Notify("Vendetta Hub", "Whitelisted — activating initializer...", 3)
--==================================================
--// SPRINT FIX + ANTI FALL DAMAGE
--==================================================

local RunBtn = nil

local function FindRunButton()
    local mg = PG:FindFirstChild("MobileGui")
    local fr = mg and mg:FindFirstChild("Frame")
    local run = fr and fr:FindFirstChild("run")
...
task.spawn(function()
    while true do
        task.wait(0.2)
        if not RunBtn or not RunBtn.Parent then
            RunBtn = FindRunButton()
        end
    end
end)

local function SetupStates(char)
    local states = char:WaitForChild("States", 10)
    if not states then return end
    states.ChildAdded:Connect(function(child)
        local n = child.Name:lower()
        if n:find("stun") or n:find("ragdoll") or n:find("frozen")
        or n:find("slow") or n:find("block")
        or n:find("restrain") or n:find("bind") then
            task.defer(function()
...
local function ResetInjury()
    local inj = LP:FindFirstChild("Injury")
    if not inj then return end
    for _, o in ipairs(inj:GetDescendants()) do
        if o:IsA("NumberValue") or o:IsA("IntValue") then
            pcall(function() o.Value = 0 end)
        end
    end
end
...
if LP.Character then SetupStates(LP.Character) end
...
RS.Heartbeat:Connect(function()
    if RunBtn and RunBtn.Parent then
        pcall(function() RunBtn:Activate() end)
    end
end)
...
print("[VENDETTA] Sprint Fix + Anti Fall Damage activated")


task.spawn(function()
    local URL = "https://pastebin.com/raw/XppvaYPH"
    ...
    local ok, result = pcall(function()
        ...
        if not fn then
            error("The Pastebin response could not be compiled.")
        end
    ...
    if ok then
        Notify("Vendetta Hub", "External initializer activated.", 4)
    else
        warn("[Vendetta Hub] External initializer failed:", result)
        Notify("Vendetta Hub", "Initializer failed to activate.", 5)
    end
end)
