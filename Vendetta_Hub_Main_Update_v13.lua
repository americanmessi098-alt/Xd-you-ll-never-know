...
--==================================================
--// ACQUISITION WINDOWS
--// Based on the working v9 acquisition flow, added to v3 base.
--==================================================
local function MakeAcquisitionWindow(Name, Title, Height)
    local Frame = Instance.new("Frame")
    Frame.Name = Name
    Frame.Size = UDim2.fromOffset(300, Height)
    Frame.Position = UDim2.new(0.5, -150, 0.5, -(Height/2))
    ...
    Header.TextSize = 14
    Header.TextColor3 = Color3.fromRGB(235,45,45)
    Header.ZIndex = 201
    ...
    Close.BackgroundColor3 = Color3.fromRGB(30,18,22)
    Close.Text = "X"
...
local function TweenAcquisition(Position)
    ...
    local Tween = TS:Create(Root, TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
...
local function RunBreathingAcquisition(TrainerName)
    ...
    task.spawn(function()
        local Ok,Err = pcall(function()
            ...
            if not TweenAcquisition(KatanaSellerPosition) then error("Character root not found") end
            task.wait(.25)
            TriggerNearbyInteractions(KatanaSellerPosition,16)
            task.wait(.75)
            local Position = TrainerPoints[TrainerName]
            if not Position then error("Unknown trainer: "..tostring(TrainerName)) end
            if not TweenAcquisition(Position) then error("Character root not found") end
            task.wait(.25)
            TriggerNearbyInteractions(Position,16)
            task.wait(.75)
            Notify("Vendetta Hub", TrainerName.." sequence finished.",2)
        end)
        AcquisitionRunning = false
        if not Ok then Notify("Vendetta Hub", "Acquisition error: "..tostring(Err),4) end
    end)
end

for _,TrainerName in ipairs(TrainerOrder) do
    local B = AcquisitionButton(BreathingBody,"Get "..TrainerName)
    B.Activated:Connect(function() RunBreathingAcquisition(TrainerName) end)
end
        ...
        if PlayerGui then
            for _,Obj in ipairs(PlayerGui:GetDescendants()) do
                if Obj:IsA("TextButton") then
                    local Text = tostring(Obj.Text):lower():gsub("%s+","")
...
local function GetOldAkumaBDA()
    ...
    task.spawn(function()
        local Ok,Err = pcall(function()
            ...
            if not TweenAcquisition(OldAkumaPosition) then error("Character root not found") end
            task.wait(1)
        ...
        AcquisitionRunning = false
        if not Ok then Notify("Vendetta Hub", "BDA error: "..tostring(Err),4) end
    end)
end
