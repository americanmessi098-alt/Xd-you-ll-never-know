...
--==================================================
--// ACQUISITION + BYPASS + ANTI FALL DAMAGE
--// Lazy-loaded so the original Vendetta GUI starts first.
--==================================================
...
do
    ...
    local function GetToolList()
        ...
        if backpack then
            for _,v in ipairs(backpack:GetChildren()) do
                if v:IsA("Tool") then found[#found+1] = v end
            end
        end
        if char then
    ...
    local function StartDesync(seconds)
        ...
        connection = RS.Heartbeat:Connect(function()
            ...
            for _,tool in ipairs(GetToolList()) do
                pcall(function()
                    hum:EquipTool(tool); hum:UnequipTools()
                    hum:EquipTool(tool); hum:UnequipTools()
                    hum:EquipTool(tool); hum:UnequipTools()
                    tool.Parent=nil; tool.Parent=backpack
                end)
            end
        end)
    ...
    local function TriggerPrompt(name)
        local prompt=FindPrompt(FindTarget(name))
    ...
    local function MoveFast(position,seconds)
        ...
        local tween=TS:Create(root,TweenInfo.new(seconds,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),{CFrame=CFrame.new(position+Vector3.new(0,3,0))})
    ...
    local function ResetInjury()
        ...
        for _,obj in ipairs(injury:GetDescendants()) do
            if obj:IsA("NumberValue") or obj:IsA("IntValue") then pcall(function() obj.Value=0 end) end
        end
    end
    ...
    local function EnableBypass()
        ...
        BypassEnabled=true
        if LP.Character then SetupBypass(LP.Character) end
        LP.CharacterAdded:Connect(function(c)
            task.wait(.5); SetupBypass(c)
        end)
        task.spawn(function()
            while BypassEnabled do
                task.wait(2); pcall(ResetInjury); pcall(FixMovement)
            end
        end)
        pcall(function()
            local pScripts=LP:FindFirstChild("PlayerScripts")
            local c=pScripts and pScripts:FindFirstChild("client")
            if c then c.Disabled=true end
        end)
    end

    RS.Heartbeat:Connect(function()
        if not FallProtectionEnabled then return end
        local char=LP.Character
        local hum=char and char:FindFirstChildOfClass("Humanoid")
        local root=char and char:FindFirstChild("HumanoidRootPart")
        if hum and root and root.Velocity.Y < -50 then
            pcall(function()
    ...
    local function MakeSelector()
        if GUI:FindFirstChild("BreathingSelector") then GUI.BreathingSelector:Destroy() end
        local frame=Instance.new("Frame")
        frame.Name="BreathingSelector"; frame.Size=UDim2.fromOffset(270,340); frame.Position=UDim2.new(.5,-135,.5,-170)
        frame.BackgroundColor3=Color3.fromRGB(11,11,17); frame.BorderSizePixel=0; frame.ZIndex=200; frame.Parent=GUI
        Instance.new("UICorner",frame).CornerRadius=UDim.new(0,12)
        local title=Instance.new("TextLabel",frame); title.Size=UDim2.new(1,-50,0,42); title.Position=UDim2.fromOffset(10,5); title.BackgroundTransparency=1; title.Text="SELECT BREATHING TRAINER"; title.TextColor3=Color3.new(1,1,1); title.Font=Enum.Font.GothamBold; title.TextSize=14; title.TextXAlignment=Enum.TextXAlignment.Left; title.ZIndex=201
        local close=Instance.new("TextButton",frame); close.Size=UDim2.fromOffset(35,35); close.Position=UDim2.new(1,-42,0,7); close.Text="X"; close.TextColor3=Color3.new(1,1,1); close.BackgroundColor3=Color3.fromRGB(35,35,45); close.BorderSizePixel=0; close.ZIndex=201; Instance.new("UICorner",close).CornerRadius=UDim.new(0,8); close.MouseButton1Click:Connect(function() frame:Destroy() end)
        local y=52
        for _,name in ipairs(TrainerOrder) do
            local b=Instance.new("TextButton",frame); b.Size=UDim2.new(1,-20,0,25); b.Position=UDim2.fromOffset(10,y); b.Text=name; b.TextColor3=Color3.new(1,1,1); b.TextSize=11; b.Font=Enum.Font.GothamMedium; b.BackgroundColor3=Color3.fromRGB(19,20,27); b.BorderSizePixel=0; b.ZIndex=201; Instance.new("UICorner",b).CornerRadius=UDim.new(0,7)
            b.MouseButton1Click:Connect(function()
                if AcquisitionBusy then return end
                AcquisitionBusy=true; frame:Destroy()
                task.spawn(function()
                    local ok,err=pcall(function()
                        local katana=FindPart(FindTarget("Katana Seller")); local trainerPos=TrainerPoints[name]
                        if not katana or not trainerPos then return end
                        MoveFast(katana.Position,3)
                        TriggerPrompt("Katana Seller")
                        task.wait(1)
                        StartDesync(2)
                        task.wait(2)
                        if DesyncConnection then DesyncConnection:Disconnect(); DesyncConnection=nil end
                    ...
                    if not ok then warn("[VENDETTA] Breathing error: "..tostring(err)) end
        ...
        frame.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch or input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; dragStart=input.Position; startPos=frame.Position end end)
        frame.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch or input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end)
        UIS.InputChanged:Connect(function(input)
            if not dragging then return end
    ...
    AutoBreathingsButton.MouseButton1Click:Connect(MakeSelector)
    ...
    AutoBDAsButton.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local ok,err=pcall(function()
                local akuma=FindPart(FindTarget("Old Akuma"))
                if not akuma then return end
                MoveFast(akuma.Position,3)
                TriggerPrompt("Old Akuma")
                StartDesync(2)
                task.wait(2)
                if DesyncConnection then DesyncConnection:Disconnect(); DesyncConnection=nil end
                task.wait(2)
            ...
            if not ok then warn("[VENDETTA] BDA error: "..tostring(err)) end
            AcquisitionBusy=false
        end)
    end)

    BypassButton.MouseButton1Click:Connect(function()
        EnableBypass(); Notify("Vendetta Hub","Bypass enabled.",2)
    end)

    AntiFallButton.MouseButton1Click:Connect(function()
        FallProtectionEnabled=not FallProtectionEnabled
        AntiFallButton.Text="Anti Fall Damage   •   "..(FallProtectionEnabled and "ON" or "OFF")
        SetToggleVisual(AntiFallButton,FallProtectionEnabled)
    end)

    print("[VENDETTA] Acquisition / bypass / anti-fall loaded.")
end
