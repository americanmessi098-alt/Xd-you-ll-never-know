...
--==================================================
--// AUTO ACQUISITION / INVENTORY DESYNC
--==================================================

local KatanaSellerPosition = Vector3.new(-251.04994201660156, 821.5532836914062, -30.124601364135742)
local AutoAcquireRunning = false
local InventoryDesyncRunning = false
local InventoryDesyncConnection = nil
local AcquisitionPopup = nil
...
local function FindPromptNear(Position, Radius)
    local Best = nil
    local BestDistance = Radius or 30
...
local function FindNamedPrompt(Name)
    local Exact = nil
    local Fallback = nil
    ...
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") then
            ...
            if ModelName == Query or ParentName == Query then
                Exact = Object
                break
            end

            if not Fallback and (ModelName:find(Query,1,true) or ParentName:find(Query,1,true)) then
                Fallback = Object
            end
        end
    end
...
local function TriggerAcquisitionPrompt(Prompt)
    ...
    local InputWorked = pcall(function()
...
local function TweenAndWait(Position)
    ...
    local ThisTween = TrainerTween
    local Completed = false
    local Connection = ThisTween.Completed:Connect(function()
        Completed = true
    end)

    while not Completed and ThisTween == TrainerTween and HRP and HRP.Parent do
        task.wait()
    end

    pcall(function() Connection:Disconnect() end)
    if TrainerTween == ThisTween then
        TrainerTween = nil
    end
...
local function StartInventoryDesync()
    if InventoryDesyncConnection then
        InventoryDesyncConnection:Disconnect()
        InventoryDesyncConnection = nil
    end

    InventoryDesyncRunning = true

    InventoryDesyncConnection = RS.Heartbeat:Connect(function()
        if not InventoryDesyncRunning then return end

        local CharacterNow = LP.Character
        if not CharacterNow then return end

        local Hum = CharacterNow:FindFirstChildOfClass("Humanoid")
...
local function CreateAcquisitionPopup()
    if AcquisitionPopup and AcquisitionPopup.Parent then
        AcquisitionPopup.Visible = true
        return AcquisitionPopup
    end

    local Popup = Instance.new("Frame")
    Popup.Name = "VendettaBreathingPicker"
    Popup.Size = UDim2.fromOffset(250,340)
    ...
    Hint.Text = "Drag this window • select a trainer"
    Hint.TextColor3 = Color3.fromRGB(135,135,145)
    ...
    for _,Name in ipairs(TrainerOrder) do
        ...
        TrainerButton.Size = UDim2.new(1,-4,0,34)
        ...
        TrainerButton.TextSize = 11
        TrainerButton.AutoButtonColor = false
        TrainerButton.ZIndex = 52
        Instance.new("UICorner",TrainerButton).CornerRadius = UDim.new(0,9)

        TrainerButton.MouseEnter:Connect(function()
            TS:Create(TrainerButton,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(45,20,25)}):Play()
        end)
        TrainerButton.MouseLeave:Connect(function()
            TS:Create(TrainerButton,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(19,20,27)}):Play()
        end)

        TrainerButton.MouseButton1Click:Connect(function()
            if AutoAcquireRunning then return end

            Popup.Visible = false
            AutoAcquireRunning = true
            AutoBreathingsButton.Text = "Getting "..Name.."..."
            SetToggleVisual(AutoBreathingsButton,true)

            task.spawn(function()
                local Success = false

                local SellerPrompt = FindPromptNear(KatanaSellerPosition,35)
                if not SellerPrompt then
                    SellerPrompt = FindNamedPrompt("Katana Seller")
                end

                if TweenAndWait(KatanaSellerPosition) then
                    task.wait(0.1)
                    SellerPrompt = SellerPrompt or FindPromptNear(KatanaSellerPosition,35)
                    if SellerPrompt then
                        TriggerAcquisitionPrompt(SellerPrompt)
                        task.wait(1)
                        if TweenAndWait(TrainerPoints[Name]) then
                ...
                AutoAcquireRunning = false
                AutoBreathingsButton.Text = "Auto Get All Breathings"
                SetToggleVisual(AutoBreathingsButton,false)

                if Success then
                    Notify("Vendetta Hub","Inventory desync started for "..Name..".",3)
                end
            end)
        end)
    end

    AcquisitionPopup = Popup
    return Popup
end

AutoBreathingsButton.MouseButton1Click:Connect(function()
    if AutoAcquireRunning then return end
    local Popup = CreateAcquisitionPopup()
    Popup.Visible = true
end)
...
AutoBDAsButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local Success = false
        local OldAkumaPrompt = FindNamedPrompt("Old Akuma")

        if OldAkumaPrompt then
            local PromptPosition = GetPromptWorldPosition(OldAkumaPrompt)
            if PromptPosition and TweenAndWait(PromptPosition) then
                TriggerAcquisitionPrompt(OldAkumaPrompt)
                StartInventoryDesync()
                task.wait(2.5)

                if OldAkumaPrompt.Parent then
                    TriggerAcquisitionPrompt(OldAkumaPrompt)
                else
                    OldAkumaPrompt = FindNamedPrompt("Old Akuma")
                    if OldAkumaPrompt then
                        TriggerAcquisitionPrompt(OldAkumaPrompt)
                    end
                end
        ...
        if Success then
            Notify("Vendetta Hub","Inventory desync started for BDA acquisition.",3)
        end
    end)
end)
