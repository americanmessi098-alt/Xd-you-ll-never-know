...
--==================================================
--// BYPASS / MOVEMENT PROTECTION ADDON
--// Added without replacing the existing Vendetta systems.
--==================================================

local BypassEnabled = false
local BypassRunButton = nil
local BypassStateConnection = nil
local BypassHeartbeatConnection = nil
local BypassCharacterConnection = nil
local BypassInjuryLoopRunning = false
...
local function SetupBypassStates(Char)
    if BypassStateConnection then
        BypassStateConnection:Disconnect()
        BypassStateConnection = nil
    end

    local States = Char:FindFirstChild("States")
    if not States then
        States = Char:WaitForChild("States", 10)
    end
    if not States then return end

    local function RemoveBadState(Child)
        if not BypassEnabled or not Child then return end

        local Name = Child.Name:lower()
        if Name:find("stun")
        or Name:find("ragdoll")
        or Name:find("frozen")
        or Name:find("slow")
        or Name:find("block")
        or Name:find("restrain")
        or Name:find("bind") then
            task.defer(function()
                if BypassEnabled and Child and Child.Parent then
                    pcall(function()
                        Child:Destroy()
                    end)
                end
    ...
    -- Remove matching states that already exist.
    for _, Child in ipairs(States:GetChildren()) do
        RemoveBadState(Child)
    end

    BypassStateConnection = States.ChildAdded:Connect(RemoveBadState)
end
...
local function StartBypass()
    ...
    BypassRunButton = FindBypassRunButton()

    -- Keep looking because MobileGui may be created after the hub.
    task.spawn(function()
        while BypassEnabled do
            task.wait(0.2)

            if not BypassRunButton or not BypassRunButton.Parent then
                BypassRunButton = FindBypassRunButton()
            end
        end
    end)

    if BypassHeartbeatConnection then
        BypassHeartbeatConnection:Disconnect()
    end

    BypassHeartbeatConnection = RS.Heartbeat:Connect(function()
        if not BypassEnabled then return end
        if not BypassRunButton or not BypassRunButton.Parent then return end

        pcall(function()
            BypassRunButton:Activate()
        end)
    end)
    ...
    if not BypassInjuryLoopRunning then
        BypassInjuryLoopRunning = true
        ...
        task.spawn(function()
            ...
            BypassInjuryLoopRunning = false
        end)
    end

    Notify("Vendetta Hub", "Bypass system enabled.", 3)
...
local function StopBypass()
    ...
    if BypassHeartbeatConnection then
        BypassHeartbeatConnection:Disconnect()
        BypassHeartbeatConnection = nil
    end

    if BypassCharacterConnection then
        BypassCharacterConnection:Disconnect()
        BypassCharacterConnection = nil
    end

    BypassRunButton = nil

    Notify("Vendetta Hub", "Bypass system disabled.", 3)
