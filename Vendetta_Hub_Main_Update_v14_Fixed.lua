...
local function RunBreathingAcquisition(TrainerName)
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            -- Inventory desync runs for exactly 3 seconds from this point.
            StartInventoryDesync()
        end)

        if not Ok then
            warn("[VENDETTA] Breathing acquisition error: ", Err)
            StopInventoryDesync()
        end
    end)
end
...
AutoBDAsButton.MouseButton1Click:Connect(function()
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            -- Inventory desync runs for exactly 3 seconds from this click.
            StartInventoryDesync()
            task.wait(2.5)

            -- Second Old Akuma interaction while the 3-second window is still active.
            TriggerPromptFor("Old Akuma")
        end)

        if not Ok then
            warn("[VENDETTA] BDA acquisition error: ", Err)
            StopInventoryDesync()
        end
    end)
end)
