...
--==================================================
-- Client-side fall protection. This is tied to the toggle below.
-- Server-authoritative damage cannot be reliably changed from the client.
...
do
    ...
    local function MoveFast(position,seconds)
        ...
        TrainerTween = TS:Create(root,TweenInfo.new(seconds,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),{
    ...
    local function EnableBypass()
        ...
        -- Disable only a clearly named local client controller if the game
        -- exposes one. Nothing is assumed about unrelated scripts.
        pcall(function()
            local pScripts = LP:FindFirstChild("PlayerScripts")
            local c = pScripts and pScripts:FindFirstChild("client")
            if c and c:IsA("LocalScript") then
                c.Disabled = true
            end
        end)
    end
    ...
    RS.Heartbeat:Connect(function()
        -- Mirror the UI toggle so Anti Fall Damage is actually optional.
        FallProtectionEnabled = AntiFallDamage
    ...
    local function MakeSelector()
        if GUI:FindFirstChild("BreathingSelector") then
            GUI.BreathingSelector:Destroy()
        end
            ...
            b.MouseButton1Click:Connect(function()
                ...
                task.spawn(function()
                    local ok,err = pcall(function()
                        ...
                        -- 1) Reach the katana seller.
                        MoveFast(katana.Position,3)
                        -- 2) Talk to the katana proximity prompt.
                        TriggerPrompt("Katana Seller")
                        -- 3) Wait exactly 1 second before desync.
                        task.wait(1)
                        -- 4) Desync for exactly 2 seconds, then stop it.
                        StartDesync(2)
                        task.wait(2)
                        if DesyncConnection then
                            DesyncConnection:Disconnect()
                        ...
                        -- 5) Tween to the selected trainer in exactly 1 second.
                        MoveFast(trainerPos,1)
                        -- 6) Talk to the trainer prompt. No extra acquisition steps.
                        TriggerPrompt(name)
                    end)
    ...
    AutoBDAsButton.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local ok,err = pcall(function()
                ...
                -- Only the first Old Akuma prompt is activated.
                TriggerPrompt("Old Akuma")
                -- Desync #1: 2 seconds, then explicitly stop.
                StartDesync(2)
                task.wait(2)
                if DesyncConnection then
                    DesyncConnection:Disconnect()
                ...
                -- Wait 2 seconds while desync is stopped.
                task.wait(2)
                -- Desync #2: 2 seconds, then explicitly stop.
                StartDesync(2)
                task.wait(2)
                if DesyncConnection then
                    DesyncConnection:Disconnect()
    ...
    print("[VENDETTA] Breathing flow: Katana prompt -> 1s -> desync 2s -> stop -> trainer tween 1s -> trainer prompt.")
    print("[VENDETTA] BDA flow: first prompt -> desync 2s -> stop -> wait 2s -> desync 2s -> stop.")
end
