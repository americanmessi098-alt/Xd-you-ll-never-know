...
--// BYPASS (client-side state/injury cleanup; does not disable the game's client)
local BypassActive = false
local BypassConnections = {}

local function AttachBypass(CharacterNow)
    local States = CharacterNow and CharacterNow:FindFirstChild("States")
    if not States then return end
    if BypassConnections.States then
        pcall(function() BypassConnections.States:Disconnect() end)
    end
    BypassConnections.States = States.ChildAdded:Connect(function(Child)
        if not BypassActive or not Child then return end
        local Name = string.lower(Child.Name)
        if string.find(Name,"stun",1,true) or string.find(Name,"ragdoll",1,true) or string.find(Name,"frozen",1,true) then
            task.defer(function()
                if BypassActive and Child.Parent then pcall(function() Child:Destroy() end) end
            end)
        end
    end)
end

local function RunBypassCleanup()
...
BypassButton.MouseButton1Click:Connect(function()
    ...
    BypassButton.Text = "Bypass   •   " .. (BypassActive and "ON" or "OFF")
    SetToggleVisual(BypassButton, BypassActive)
    if BypassActive then
        AttachBypass(LP.Character)
        RunBypassCleanup()
        Notify("Vendetta Hub","Bypass enabled.",2)
    else
        Notify("Vendetta Hub","Bypass disabled.",2)
    end
end)
...
task.spawn(function()
    while GUI.Parent do
        if BypassActive then pcall(RunBypassCleanup) end
        task.wait(2)
    end
end)
...
RS.Heartbeat:Connect(function()
    ...
    if Velocity.Y < -50 then
        ...
        pcall(function() Root.AssemblyLinearVelocity = Vector3.new(Velocity.X,-10,Velocity.Z) end)
    end
end)
...
--==================================================
--// ACQUISITION HELPERS
--// Separate worker logic so the main hub is not held up by waits.
--==================================================
do
    ...
    local function ActivatePrompt(Object)
    ...
    local function TweenTo(Position, Duration)
        ...
        local Goal = CFrame.new(Position + Vector3.new(0,3,0))
        local Tween = TS:Create(Root,TweenInfo.new(Duration,Enum.EasingStyle.Linear),{CFrame=Goal})
        Tween:Play()
        Tween.Completed:Wait()
        return true
    end

    -- Desync pulse: repeatedly moves the local tool state between character/backpack.
    -- It is isolated from the GUI thread and always stops on its own.
    local function DesyncPulse(Duration)
        if DesyncRunning then return end
        DesyncRunning = true
        local Deadline = os.clock() + Duration
        task.spawn(function()
            while DesyncRunning and os.clock() < Deadline do
                local C = LP.Character
                local H = C and C:FindFirstChildOfClass("Humanoid")
                local Backpack = LP:FindFirstChildOfClass("Backpack")
                if H and Backpack then
    ...
    local function RunBreathing(TrainerName)
        local KatanaSeller = FindNamed("Katana Seller")
        local TrainerPosition = TrainerPoints[TrainerName]
        local KatanaPart = GetPart(KatanaSeller)
        if not KatanaPart or not TrainerPosition then
            Notify("Vendetta Hub","Katana Seller or trainer not found.",3)
            return
        end

        -- Requested order: reach Katana -> first prompt -> wait 1s -> desync 2s -> stop -> trainer in 1s -> trainer prompt.
        if not TweenTo(KatanaPart.Position,3) then return end
        task.wait(1)
        ActivatePrompt(KatanaSeller)
        DesyncPulse(2)
        TweenTo(TrainerPosition,1)

        local TrainerObject = FindNamed(TrainerName)
        if TrainerObject then
            ActivatePrompt(TrainerObject)
        end
    end

    local function RunBDA()
        local Akuma = FindNamed("Old Akuma")
        local AkumaPart = GetPart(Akuma)
        if not AkumaPart then
            Notify("Vendetta Hub","Old Akuma not found.",3)
            return
        end

        -- Requested order: Old Akuma -> first prompt only -> desync 2s -> stop -> wait 2s -> desync 2s -> stop.
        if not TweenTo(AkumaPart.Position,3) then return end
        task.wait(.15)
        ActivatePrompt(Akuma)
        DesyncPulse(2)
        task.wait(2)
        DesyncPulse(2)
    end

    local function OpenBreathingSelector()
        ...
        Close.Position = UDim2.new(1,-41,0,6)
        Close.Text = "X"
        ...
        for _,TrainerName in ipairs(TrainerOrder) do
            local ButtonObject = Instance.new("TextButton",List)
            ButtonObject.Size = UDim2.new(1,-4,0,26)
            ButtonObject.Text = TrainerName
            ButtonObject.TextColor3 = Color3.new(1,1,1)
            ButtonObject.TextSize = 11
            ButtonObject.Font = Enum.Font.GothamMedium
            ButtonObject.BackgroundColor3 = Color3.fromRGB(19,20,27)
            ButtonObject.BorderSizePixel = 0
            ButtonObject.ZIndex = 202
            Instance.new("UICorner",ButtonObject).CornerRadius = UDim.new(0,7)
            ButtonObject.MouseButton1Click:Connect(function()
                if AcquisitionBusy then return end
                AcquisitionBusy = true
                Frame:Destroy()
                task.spawn(function()
                    local Ok,Err = pcall(RunBreathing,TrainerName)
        ...
        -- Touch/mouse dragging without using the main hub's drag connection.
    ...
    AutoBreathingsButton.MouseButton1Click:Connect(OpenBreathingSelector)

    AutoBDAsButton.MouseButton1Click:Connect(function()
        if AcquisitionBusy then return end
        AcquisitionBusy = true
        task.spawn(function()
            local Ok,Err = pcall(RunBDA)
            if not Ok then warn("[VENDETTA] BDA error: "..tostring(Err)) end
            AcquisitionBusy = false
        end)
    end)

    print("[VENDETTA] Remade acquisition system loaded.")
end
