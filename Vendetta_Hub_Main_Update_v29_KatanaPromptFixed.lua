...
task.defer(function()
    local ok, err = pcall(function()
        ...
        local function firePrompt(prompt)
            ...
            -- Primary method used by supported executors.
            ...
            -- Fallback: simulate the complete hold interaction.
            local ok = pcall(function()
                ...
                local duration = math.max(0, prompt.HoldDuration or 0)
                if duration > 0 then
            ...
            for _, trainerName in ipairs(TrainerOrder) do
                ...
                button.MouseButton1Click:Connect(function()
                    ...
                    task.spawn(function()
                        ...
                        -- Wait for the Katana tween to actually finish.
                        if not tweenAndWait(KatanaSellerPosition, 3) then return end
                        task.wait(.1)
                        usePrompt("Katana Seller")

                        -- Desync for 1.5 seconds, then give it another 0.5 second gap.
                        desyncFor(1.5)
                        task.wait(1.5)
                        stopDesync()
                        task.wait(.5)

                        -- Phase 2: trainer trip is always exactly 1.5 seconds.
                        if not tweenAndWait(position, 1.5) then return end
                        task.wait(.1)
