...
GUI.Parent = PG
print("[VENDETTA] GUI created successfully")
...
local function StartInventoryDesync(Duration)
    ...
    -- The caller can choose the lifetime. BDA uses two separate 2-second bursts.
...
AutoBDAsButton.MouseButton1Click:Connect(function()
    task.spawn(function()
        ...
        -- Reach Old Akuma first.
        TweenAndWait(OldAkumaPosition)
        TriggerPromptFor("Old Akuma")

        -- First desync burst: exactly 2 seconds.
        StartInventoryDesync(2)
        task.wait(2)

        -- Pause for exactly 2 seconds.
        task.wait(2)

        -- Interact with Old Akuma again, then start the second burst.
        TriggerPromptFor("Old Akuma")
        StartInventoryDesync(2)
    end)
end)
...
MakeDraggable(Main)
print("[VENDETTA] Main draggable initialized")
...
print("[VENDETTA] STARTUP COMPLETE")
