...
-- Automation placeholders: the requested inventory-desync acquisition routine is intentionally not included.
...
--==================================================
--// AUTO BREATHING / BDA ACQUISITION V15
--// Added after the original V12 code so startup is unchanged.
--==================================================

local V15_KatanaSellerPosition = Vector3.new(-251.04994201660156, 821.5532836914062, -30.124601364135742)
local V15_AutoBreathingBusy = false
local V15_AutoBDABusy = false
local V15_DesyncRunning = false
local V15_DesyncConnection = nil
local V15_BreathingMenu = nil

local V15_BreathingTrainers = {
    "Rock Trainer",
    "Water Trainer",
    "Beast Trainer",
    "Thunder Trainer",
...
local function V15_GetPromptPart(Prompt)
...
local function V15_FindPromptNear(Position, MaxDistance)
    local Best = nil
    local BestDistance = MaxDistance or math.huge

    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") then
            local Part = V15_GetPromptPart(Object)
            if Part then
...
local function V15_FindPromptByName(Name)
...
local function V15_FindKatanaPrompt()
    -- First try common exact prompt names.
    for _,Name in ipairs({"KatanaSeller", "Katana Seller", "KatanaPrompt", "BuyKatana", "Katana"}) do
        local Prompt = V15_FindPromptByName(Name)
        if Prompt then return Prompt end
    end

    -- Then only accept a prompt very close to the supplied seller coordinates.
    return V15_FindPromptNear(V15_KatanaSellerPosition, 35)
end

local function V15_FindOldAkumaPrompt()
    -- Try exact prompt names first.
    for _,Name in ipairs({"OldAkuma", "Old Akuma", "AkumaPrompt", "OldAkumaPrompt"}) do
        local Prompt = V15_FindPromptByName(Name)
        if Prompt then return Prompt end
    end

    -- Then inspect prompt ancestry for Old Akuma.
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") then
            local Parent = Object.Parent
            local Model = Object:FindFirstAncestorOfClass("Model")
            local ParentName = string.lower(Parent and Parent.Name or "")
            local ModelName = string.lower(Model and Model.Name or "")

            if string.find(ParentName, "oldakuma", 1, true)
                or string.find(ParentName, "old akuma", 1, true)
                or string.find(ModelName, "oldakuma", 1, true)
                or string.find(ModelName, "old akuma", 1, true) then
                return Object
            end
        end
    end
...
local function V15_TriggerPrompt(Prompt)
...
local function V15_TweenAndWait(Position)
    ...
    while not Completed and os.clock() - Started < Duration + 2 do
        if not Root.Parent then break end
        task.wait()
    end

    Connection:Disconnect()
    if TrainerTween == Tween then
        TrainerTween = nil
    end
...
local function V15_GetAllTools()
    local Found = {}
    local Backpack = LP:FindFirstChildOfClass("Backpack")
    ...
    if CharacterNow then
        for _,Tool in ipairs(CharacterNow:GetChildren()) do
            if Tool:IsA("Tool") then
                Found[#Found + 1] = Tool
            end
        end
...
local function V15_StopDesync()
    V15_DesyncRunning = false

    if V15_DesyncConnection then
        V15_DesyncConnection:Disconnect()
        V15_DesyncConnection = nil
    end
end

local function V15_StartDesync()
    V15_StopDesync()
    V15_DesyncRunning = true

    V15_DesyncConnection = RS.Heartbeat:Connect(function()
        if not V15_DesyncRunning then return end

        local CharacterNow = LP.Character
        local HumanoidNow = CharacterNow and CharacterNow:FindFirstChildOfClass("Humanoid")
        local Backpack = LP:FindFirstChildOfClass("Backpack")
        if not HumanoidNow or not Backpack then return end

        for _,Tool in ipairs(V15_GetAllTools()) do
            if Tool and Tool.Parent then
                pcall(function()
                    HumanoidNow:EquipTool(Tool)
                    HumanoidNow:UnequipTools()
                    HumanoidNow:EquipTool(Tool)
                    HumanoidNow:UnequipTools()
                    HumanoidNow:EquipTool(Tool)
                    HumanoidNow:UnequipTools()

                    -- Only restore a Tool that still exists; never leave it nil-parented.
                    if Tool.Parent then
...
local function V15_SetButtonState(ButtonObject, Text, Enabled)
    if not ButtonObject or not ButtonObject.Parent then return end
    ButtonObject.Text = Text
    pcall(function()
        SetToggleVisual(ButtonObject, Enabled)
    end)
end

local function V15_MakeDraggable(Frame)
    local Dragging = false
    local DragStart = nil
...
local function V15_CloseBreathingMenu()
    if V15_BreathingMenu then
        V15_BreathingMenu:Destroy()
        V15_BreathingMenu = nil
    end
end

local function V15_OpenBreathingMenu()
    if V15_BreathingMenu then
        V15_CloseBreathingMenu()
        return
    end

    local Menu = Instance.new("Frame")
    Menu.Name = "BreathingTrainerSelector"
    Menu.Size = UDim2.fromOffset(270, 330)
    Menu.Position = UDim2.new(0.5, -135, 0.5, -165)
    Menu.BackgroundColor3 = Color3.fromRGB(13,14,20)
    ...
    Title.Text = "SELECT BREATHING TRAINER"
    ...
    Close.BackgroundColor3 = Color3.fromRGB(30,31,40)
    ...
    Close.Activated:Connect(V15_CloseBreathingMenu)
    ...
    for Index,TrainerName in ipairs(V15_BreathingTrainers) do
        local TrainerButton = Instance.new("TextButton", List)
        TrainerButton.Size = UDim2.new(1,-6,0,40)
        TrainerButton.BackgroundColor3 = Color3.fromRGB(20,21,29)
        TrainerButton.BorderSizePixel = 0
        TrainerButton.Text = TrainerName
        TrainerButton.TextColor3 = Color3.fromRGB(240,240,245)
        TrainerButton.Font = Enum.Font.GothamSemibold
        TrainerButton.TextSize = 12
        TrainerButton.AutoButtonColor = false
        TrainerButton.LayoutOrder = Index
        TrainerButton.ZIndex = 52
        Instance.new("UICorner", TrainerButton).CornerRadius = UDim.new(0,9)

        TrainerButton.Activated:Connect(function()
            if V15_AutoBreathingBusy then return end
            local Destination = TrainerPoints[TrainerName]
            if not Destination then
                Notify("Vendetta Hub", "Trainer location not found.", 3)
                return
            end

            V15_AutoBreathingBusy = true
            V15_SetButtonState(TrainerButton, "Starting...", true)
            V15_CloseBreathingMenu()

            task.spawn(function()
                local SellerPrompt = V15_FindKatanaPrompt()
                if not SellerPrompt then
                    Notify("Vendetta Hub", "Katana seller prompt not found.", 4)
                    V15_AutoBreathingBusy = false
                    return
                end

                -- Tween to the exact Katana Seller location.
                if not V15_TweenAndWait(V15_KatanaSellerPosition) then
                    Notify("Vendetta Hub", "Katana seller tween did not finish.", 4)
                    V15_AutoBreathingBusy = false
                    return
                end
                ...
                SellerPrompt = V15_FindKatanaPrompt() or SellerPrompt
                if not V15_TriggerPrompt(SellerPrompt) then
                    Notify("Vendetta Hub", "Could not trigger Katana Seller prompt.", 4)
                    V15_AutoBreathingBusy = false
                    return
                end

                -- Start the supplied routine without creating its original GUI.
                V15_StartDesync()
                task.wait(1)

                if not V15_TweenAndWait(Destination) then
                    Notify("Vendetta Hub", "Trainer tween did not finish.", 4)
                end

                V15_StopDesync()
                V15_AutoBreathingBusy = false
                Notify("Vendetta Hub", "Finished: " .. TrainerName, 3)
            end)
        end)
    end

    V15_BreathingMenu = Menu
    V15_MakeDraggable(Menu)
end

AutoBreathingsButton.MouseButton1Click:Connect(V15_OpenBreathingMenu)

AutoBDAsButton.MouseButton1Click:Connect(function()
    if V15_AutoBDABusy then return end
    V15_AutoBDABusy = true
    V15_SetButtonState(AutoBDAsButton, "Getting BDAs...", true)

    task.spawn(function()
        local OldAkumaPrompt = V15_FindOldAkumaPrompt()
        if not OldAkumaPrompt then
            Notify("Vendetta Hub", "Old Akuma prompt not found.", 4)
            V15_AutoBDABusy = false
            V15_SetButtonState(AutoBDAsButton, "Auto Get All BDAs", false)
            return
        end

        local AkumaPart = V15_GetPromptPart(OldAkumaPrompt)
        if not AkumaPart then
            Notify("Vendetta Hub", "Old Akuma position not found.", 4)
            V15_AutoBDABusy = false
            V15_SetButtonState(AutoBDAsButton, "Auto Get All BDAs", false)
            return
        end

        if not V15_TweenAndWait(AkumaPart.Position) then
            Notify("Vendetta Hub", "Old Akuma tween did not finish.", 4)
            V15_AutoBDABusy = false
            V15_SetButtonState(AutoBDAsButton, "Auto Get All BDAs", false)
            return
        end

        OldAkumaPrompt = V15_FindOldAkumaPrompt() or OldAkumaPrompt
        if not V15_TriggerPrompt(OldAkumaPrompt) then
            Notify("Vendetta Hub", "Could not trigger Old Akuma prompt.", 4)
            V15_AutoBDABusy = false
            V15_SetButtonState(AutoBDAsButton, "Auto Get All BDAs", false)
            return
        end
        ...
        OldAkumaPrompt = V15_FindOldAkumaPrompt() or OldAkumaPrompt
        V15_TriggerPrompt(OldAkumaPrompt)
        V15_StopDesync()

        V15_AutoBDABusy = false
        V15_SetButtonState(AutoBDAsButton, "Auto Get All BDAs", false)
        Notify("Vendetta Hub", "BDA sequence finished.", 3)
    end)
end)
