...
FluteTween.MouseButton1Click:Connect(function()
    ...
    if not Flute then
        Notify("Vendetta Hub", "Flute not found in workspace.chatnpcs.", 3)
        return
    end
    TweenTo(Flute)
end)
...
local function GetAllToolsForDesync()
    local Found = {}
    local Backpack = LP:FindFirstChildOfClass("Backpack")
    if Backpack then
        for _, Tool in ipairs(Backpack:GetChildren()) do
            if Tool:IsA("Tool") then Found[#Found+1] = Tool end
        end
    end
...
local function StartDesyncSpam()
    ...
    DesyncConnection = RS.Heartbeat:Connect(function()
        ...
        for _, Tool in ipairs(GetAllToolsForDesync()) do
            pcall(function()
                Hum:EquipTool(Tool)
                Hum:UnequipTools()
                Hum:EquipTool(Tool)
...
BDANote.Text = "Old Akuma acquisition is ready for the window, but the BDA names and Old Akuma location were not supplied."
BDANote.Font = Enum.Font.GothamMedium
BDANote.TextSize = 11
BDANote.TextColor3 = Color3.fromRGB(160,160,170)

local OldAkumaButton = AcquisitionButton(BDABody, "Old Akuma — BDA Acquisition")
OldAkumaButton.Activated:Connect(function()
    Notify("Vendetta Hub", "Old Akuma location/BDA sequence is not configured.", 3)
end)
