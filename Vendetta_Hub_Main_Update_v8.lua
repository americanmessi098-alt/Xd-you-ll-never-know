...
if not LP then
    warn("[VENDETTA] LocalPlayer is not ready.")
    return
end

-- Do not block the entire script waiting for the character. Some executors
-- can start before the character/PlayerGui finishes replicating.
local PG = LP:FindFirstChildOfClass("PlayerGui") or LP:WaitForChild("PlayerGui", 10)
if not PG then
    warn("[VENDETTA] PlayerGui was not ready.")
    return
end
...
local function RefreshCharacterReferences(C)
    Character = C
...
if Character then
    RefreshCharacterReferences(Character)
end
...
-- Acquisition windows are created below after the main helpers are loaded.
...
local function FindEarrings()
    -- Exact logic from the supplied working Earring Finder:
    -- workspace.map:GetChildren() -> Model named Barrel.
...
BypassButton.MouseButton1Click:Connect(function()
    Notify("Vendetta Hub", "Bypass is disabled in this build.", 4)
end)
...
local function StartAntiFall()
    ...
    AntiFallConnection = RS.Heartbeat:Connect(function()
        if not AntiFallDamage then return end

        local CharacterNow = LP.Character
        if not CharacterNow then return end

        local HumanoidNow = CharacterNow:FindFirstChildOfClass("Humanoid")
        local RootPart = CharacterNow:FindFirstChild("HumanoidRootPart")
        if not HumanoidNow or not RootPart then return end

        if HumanoidNow:GetState() == Enum.HumanoidStateType.Freefall then
            local Velocity = RootPart.Velocity.Y
            if Velocity < -50 then
                HumanoidNow:ChangeState(Enum.HumanoidStateType.RunningNoPhysics)
                local V = RootPart.Velocity
                RootPart.Velocity = Vector3.new(V.X, -10, V.Z)
            end
        end
    end)
end
...
--==================================================
--// AUTO ACQUISITION WINDOWS
--// Clean implementation: no inventory desync and no anti-cheat bypass.
--// Both windows are draggable/minimizable and support Tween/Teleport.
--==================================================

local AcquisitionGui = Instance.new("ScreenGui")
AcquisitionGui.Name = "VendettaAcquisition"
AcquisitionGui.ResetOnSpawn = false
AcquisitionGui.IgnoreGuiInset = true
AcquisitionGui.DisplayOrder = 1000001
AcquisitionGui.Parent = PG

local function MakeAcquisitionWindow(Name, TitleText, Height)
    local Frame = Instance.new("Frame")
    Frame.Name = Name
    Frame.Size = UDim2.fromOffset(285, Height)
    Frame.Position = UDim2.new(0.5, -142, 0.5, -Height/2)
    Frame.BackgroundColor3 = Color3.fromRGB(12,12,18)
    Frame.BorderSizePixel = 0
    Frame.Active = true
    Frame.Parent = AcquisitionGui
    ...
    Min.Position = UDim2.new(1,-40,0,6)
    Min.Text = "—"
    Min.Font = Enum.Font.GothamBold
    Min.TextSize = 18
    Min.TextColor3 = Color3.fromRGB(235,45,45)
    Min.BackgroundColor3 = Color3.fromRGB(25,20,27)
    ...
    Body.Position = UDim2.fromOffset(6,48)
    Body.Size = UDim2.new(1,-12,1,-54)
    ...
    Min.Activated:Connect(function()
        ...
        Frame.Size = Minimized and UDim2.fromOffset(285,42) or NormalSize
...
local function AcquisitionButton(Parent, Text)
    local B = Instance.new("TextButton", Parent)
    ...
    Instance.new("UICorner", B).CornerRadius = UDim.new(0,9)
    B.MouseEnter:Connect(function()
        TS:Create(B,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(30,20,27)}):Play()
    end)
    B.MouseLeave:Connect(function()
        TS:Create(B,TweenInfo.new(.12),{BackgroundColor3=Color3.fromRGB(19,20,27)}):Play()
    end)
...
local BreathingWindow, BreathingBody = MakeAcquisitionWindow(
    "BreathingWindow", "AUTO GET BREATHINGS", 360
)
local BDAWindow, BDABody = MakeAcquisitionWindow(
    "BDAWindow", "AUTO GET BDAs", 240
)
BreathingWindow.Visible = false
BDAWindow.Visible = false

local KatanaSellerPosition = Vector3.new(
    -251.04994201660156, 821.5532836914062, -30.124601364135742
)
local OldAkumaPosition = Vector3.new(-1097.89636, 824.803345, -347.040924)

local BreathingMode = "Tween"
local BDAMode = "Tween"
local AcquisitionRunning = false

local function MoveAcquisition(Position, Mode)
    ...
    -- Acquisition tween is always exactly 2 seconds.
...
local function TriggerNearbyInteractions(Position, Radius)
    Radius = Radius or 16
    local Fired = 0
...
local function AutoAcceptDialogOnce(Duration)
    local Deadline = os.clock() + (Duration or 5)
            ...
            for _, GuiObject in ipairs(PlayerGui:GetDescendants()) do
                if GuiObject:IsA("TextButton") or GuiObject:IsA("ImageButton") then
                    ...
                    if GuiObject:IsA("TextButton") then
                        Text = tostring(GuiObject.Text):lower():gsub("%s+", "")
                    end
                    if Text == "yes" or Text == "accept" or Text == "ok" then
...
local function InteractWithOldAkuma()
    local NPC = workspace:FindFirstChild("Old Akuma", true)
    if not NPC then
        Notify("Vendetta Hub", "Old Akuma was not found.", 3)
        return false
    end

    local Prompt = NPC:FindFirstChildWhichIsA("ProximityPrompt", true)
    if not Prompt then
        Notify("Vendetta Hub", "No Old Akuma ProximityPrompt found.", 3)
        return false
    end

    pcall(function() fireproximityprompt(Prompt) end)
    task.spawn(function()
        AutoAcceptDialogOnce(5)
    end)
    return true
end

local function RunBreathingAcquisition(TrainerName)
    if AcquisitionRunning then
        Notify("Vendetta Hub", "An acquisition is already running.", 3)
        return
    end

    local TrainerPosition = TrainerPoints[TrainerName]
    if not TrainerPosition then
        Notify("Vendetta Hub", "Trainer position is missing.", 3)
        return
    end
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            ...
            -- Step 1: Katana Seller.
            MoveAcquisition(KatanaSellerPosition, BreathingMode)
            task.wait(.3)
            TriggerNearbyInteractions(KatanaSellerPosition, 16)
            task.wait(.8)

            -- Step 2: Selected breathing trainer.
            MoveAcquisition(TrainerPosition, BreathingMode)
            task.wait(.3)
            TriggerNearbyInteractions(TrainerPosition, 16)
            task.wait(.8)

            Notify("Vendetta Hub", TrainerName.." sequence finished.", 2)
        end)

        AcquisitionRunning = false
        if not Ok then
            Notify("Vendetta Hub", "Breathing error: "..tostring(Err), 4)
        end
    end)
end

local BreathingModeLabel = Instance.new("TextLabel", BreathingBody)
BreathingModeLabel.Size = UDim2.new(1,-4,0,24)
BreathingModeLabel.BackgroundTransparency = 1
BreathingModeLabel.Text = "Movement Mode: TWEEN"
BreathingModeLabel.Font = Enum.Font.GothamSemibold
BreathingModeLabel.TextSize = 11
BreathingModeLabel.TextColor3 = Color3.fromRGB(220,220,225)

local BreathingModeButton = AcquisitionButton(BreathingBody, "Mode: TWEEN")
BreathingModeButton.Activated:Connect(function()
    BreathingMode = BreathingMode == "Tween" and "Teleport" or "Tween"
    BreathingModeButton.Text = "Mode: "..string.upper(BreathingMode)
    BreathingModeLabel.Text = "Movement Mode: "..string.upper(BreathingMode)
end)

for _, TrainerName in ipairs(TrainerOrder) do
    local B = AcquisitionButton(BreathingBody, "Get "..TrainerName)
    B.Activated:Connect(function()
        RunBreathingAcquisition(TrainerName)
    end)
end

local BDAModeLabel = Instance.new("TextLabel", BDABody)
BDAModeLabel.Size = UDim2.new(1,-4,0,24)
BDAModeLabel.BackgroundTransparency = 1
BDAModeLabel.Text = "Movement Mode: TWEEN"
BDAModeLabel.Font = Enum.Font.GothamSemibold
BDAModeLabel.TextSize = 11
BDAModeLabel.TextColor3 = Color3.fromRGB(220,220,225)

local BDAModeButton = AcquisitionButton(BDABody, "Mode: TWEEN")
BDAModeButton.Activated:Connect(function()
    BDAMode = BDAMode == "Tween" and "Teleport" or "Tween"
    BDAModeButton.Text = "Mode: "..string.upper(BDAMode)
    BDAModeLabel.Text = "Movement Mode: "..string.upper(BDAMode)
end)

local BDANote = Instance.new("TextLabel", BDABody)
BDANote.Size = UDim2.new(1,-4,0,45)
BDANote.BackgroundTransparency = 1
BDANote.TextWrapped = true
BDANote.Text = "Old Akuma only • automatic prompt + dialog handling"
BDANote.Font = Enum.Font.GothamMedium
BDANote.TextSize = 11
BDANote.TextColor3 = Color3.fromRGB(160,160,170)

local OldAkumaButton = AcquisitionButton(BDABody, "Get Old Akuma BDA")
OldAkumaButton.Activated:Connect(function()
    if AcquisitionRunning then
        Notify("Vendetta Hub", "An acquisition is already running.", 3)
        return
    end
    ...
    task.spawn(function()
        local Ok, Err = pcall(function()
            Notify("Vendetta Hub", "Going to Old Akuma...", 2)
            MoveAcquisition(OldAkumaPosition, BDAMode)
            task.wait(1)
            InteractWithOldAkuma()
            task.wait(.5)
            Notify("Vendetta Hub", "Old Akuma sequence finished.", 2)
        end)

        AcquisitionRunning = false
        if not Ok then
            Notify("Vendetta Hub", "BDA error: "..tostring(Err), 4)
        end
    end)
end)

AutoBreathingsButton.Activated:Connect(function()
    BreathingWindow.Visible = not BreathingWindow.Visible
    BDAWindow.Visible = false
end)

AutoBDAsButton.Activated:Connect(function()
    BDAWindow.Visible = not BDAWindow.Visible
    BreathingWindow.Visible = false
end)
