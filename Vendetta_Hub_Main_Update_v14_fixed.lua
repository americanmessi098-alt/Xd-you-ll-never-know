...
--==================================================
--// AUTO BREATHING / BDA ACQUISITION
--// Uses the existing trainer tween system and the supplied
--// desync routine without creating a second GUI for it.
--==================================================

local KatanaSellerPosition = Vector3.new(-251.04994201660156, 821.5532836914062, -30.124601364135742)
local AutoBreathingMenu = nil
local AutoBreathingBusy = false
local AutoBDABusy = false
...
local function FindPromptByName(Name)
    ...
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") and string.lower(Object.Name) == Wanted then
            return Object
        end
    end
    return nil
...
local function FindKatanaSellerPrompt()
    -- Prefer a prompt explicitly named for the katana seller.
    for _,Name in ipairs({"KatanaSeller", "Katana Seller", "KatanaPrompt", "BuyKatana"}) do
        local Prompt = FindPromptByName(Name)
        if Prompt then return Prompt end
    end

    -- Otherwise use the nearest ProximityPrompt to the supplied seller coordinates.
    local BestPrompt = nil
    local BestDistance = math.huge
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") then
            local Root = GetPromptRoot(Object)
            if Root then
                local Distance = (Root.Position - KatanaSellerPosition).Magnitude
...
local function FindOldAkumaPrompt()
    local ExactNames = {"Old Akuma", "OldAkuma", "AkumaPrompt"}
    for _,Name in ipairs(ExactNames) do
        local Prompt = FindPromptByName(Name)
        if Prompt then return Prompt end
    end

    -- Also check the prompt's parent/model names.
    for _,Object in ipairs(workspace:GetDescendants()) do
        if Object:IsA("ProximityPrompt") then
            ...
            if string.find(ParentName, "old akuma", 1, true)
                or string.find(ParentName, "oldakuma", 1, true)
                or string.find(ModelName, "old akuma", 1, true)
                or string.find(ModelName, "oldakuma", 1, true) then
                return Object
            end
        end
    end
...
local function StopAcquisitionDesync()
    DesyncRunning = false
...
local function StartAcquisitionDesync()
    StopAcquisitionDesync()
    DesyncRunning = true
...
local AutoTrainerOrder = {
    "Rock Trainer", "Water Trainer", "Beast Trainer", "Thunder Trainer",
    "Serpent Trainer", "Flame Trainer", "Flower Trainer", "Wind Trainer",
    "Rat Trainer", "Love Trainer"
}

local function CloseAutoBreathingMenu()
    if AutoBreathingMenu then
        AutoBreathingMenu:Destroy()
        AutoBreathingMenu = nil
    end
end

local function OpenAutoBreathingMenu()
    if AutoBreathingMenu then
        CloseAutoBreathingMenu()
        return
    end

    local Menu = Instance.new("Frame")
    AutoBreathingMenu = Menu
    Menu.Name = "AutoBreathingTrainerMenu"
    Menu.Size = UDim2.fromOffset(285, 390)
    Menu.Position = UDim2.new(.5, -142, .5, -195)
    ...
    Border.Color = Color3.fromRGB(235,45,45)
    Border.Thickness = 2

    local Header = Instance.new("TextLabel", Menu)
    Header.Size = UDim2.new(1,-55,0,38)
    Header.Position = UDim2.fromOffset(14,7)
    Header.BackgroundTransparency = 1
    Header.Text = "SELECT BREATHING TRAINER"
    ...
    Close.TextColor3 = Color3.fromRGB(235,45,45)
    Close.BackgroundColor3 = Color3.fromRGB(25,20,25)
    Close.BorderSizePixel = 0
    Close.ZIndex = 51
    Instance.new("UICorner", Close).CornerRadius = UDim.new(0,8)
    Close.Activated:Connect(CloseAutoBreathingMenu)
    ...
    for Index,TrainerName in ipairs(AutoTrainerOrder) do
        local B = Instance.new("TextButton", List)
        B.Size = UDim2.new(1,-6,0,40)
        B.Position = UDim2.fromOffset(3,(Index-1)*44)
        B.Text = TrainerName
        B.TextSize = 12
        B.Font = Enum.Font.GothamSemibold
        B.TextColor3 = Color3.fromRGB(240,240,245)
        ...
        B.Activated:Connect(function()
            if AutoBreathingBusy then return end
            AutoBreathingBusy = true
            B.Text = "Starting..."

            task.spawn(function()
                local SellerPrompt = FindKatanaSellerPrompt()
                if not SellerPrompt then
                    Notify("Vendetta Hub", "Katana seller proximity prompt not found.", 4)
                    AutoBreathingBusy = false
                    return
                end

                CloseAutoBreathingMenu()

                -- 1) Tween to the katana seller and wait for the tween to finish.
                if not TweenAndWait(KatanaSellerPosition) then
                    AutoBreathingBusy = false
                    return
                end

                -- 2) Give the seller a moment, then trigger its prompt.
                task.wait(1.5)
                SellerPrompt = FindKatanaSellerPrompt() or SellerPrompt
                if not TriggerProximityPrompt(SellerPrompt) then
                    Notify("Vendetta Hub", "Could not trigger the katana seller prompt.", 4)
                    AutoBreathingBusy = false
                    return
                end

                -- 3) Start the supplied desync routine, with no extra GUI.
                StartAcquisitionDesync()
                task.wait(1)

                -- 4) Tween to the selected breathing trainer.
                local TrainerPosition = TrainerPoints[TrainerName]
                if TrainerPosition then
                    TweenAndWait(TrainerPosition)
                end
...
AutoBreathingsButton.MouseButton1Click:Connect(OpenAutoBreathingMenu)

AutoBDAsButton.MouseButton1Click:Connect(function()
    if AutoBDABusy then return end
    AutoBDABusy = true
    AutoBDAsButton.Text = "Getting BDAs..."
    SetToggleVisual(AutoBDAsButton, true)

    task.spawn(function()
        local OldAkumaPrompt = FindOldAkumaPrompt()
        if not OldAkumaPrompt then
            Notify("Vendetta Hub", "Old Akuma proximity prompt not found.", 4)
            AutoBDABusy = false
            AutoBDAsButton.Text = "Auto Get All BDAs"
            SetToggleVisual(AutoBDAsButton, false)
            return
        end

        local PromptRoot = GetPromptRoot(OldAkumaPrompt)
        if not PromptRoot then
            Notify("Vendetta Hub", "Old Akuma prompt position could not be found.", 4)
            AutoBDABusy = false
            AutoBDAsButton.Text = "Auto Get All BDAs"
            SetToggleVisual(AutoBDAsButton, false)
            return
        ...
        -- 1) Tween to Old Akuma.
        if not TweenAndWait(PromptRoot.Position) then
            AutoBDABusy = false
            AutoBDAsButton.Text = "Auto Get All BDAs"
            SetToggleVisual(AutoBDAsButton, false)
            return
        ...
        -- 2) Trigger Old Akuma once.
        OldAkumaPrompt = FindOldAkumaPrompt() or OldAkumaPrompt
        if not TriggerProximityPrompt(OldAkumaPrompt) then
        ...
        -- 3) Start the supplied desync routine, wait 2.5 seconds, then trigger again.
        StartAcquisitionDesync()
        task.wait(2.5)
        OldAkumaPrompt = FindOldAkumaPrompt() or OldAkumaPrompt
        TriggerProximityPrompt(OldAkumaPrompt)
        StopAcquisitionDesync()
