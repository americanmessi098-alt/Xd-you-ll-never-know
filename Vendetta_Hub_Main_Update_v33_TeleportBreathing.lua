...
local function SetupBypassStates(Char)
    ...
    -- Prevent duplicate connections if this character is initialized again.
    if BypassStateConnections[Char] then
        pcall(function()
            BypassStateConnections[Char]:Disconnect()
        end)
    end

    BypassStateConnections[Char] = States.ChildAdded:Connect(function(Child)
...
local function SetBypass(State)
    ...
    BypassButton.Text = "Bypass   •   " .. (BypassEnabled and "ON" or "OFF")
    SetToggleVisual(BypassButton, BypassEnabled)
    ...
    if BypassEnabled then
        ...
        Notify("Vendetta Hub", "Bypass / Sprint Fix enabled.", 3)
    else
        Notify("Vendetta Hub", "Bypass / Sprint Fix disabled.", 3)
    end
end
...
-- Keep searching because MobileGui/run can be created after the hub starts.
task.spawn(function()
    while true do
        task.wait(0.2)
        if BypassEnabled and (not RunBtn or not RunBtn.Parent) then
            RunBtn = FindRunButton()
        end
    end
end)

-- Activate the mobile run button while the bypass is enabled.
...
-- Continuously repair movement/injury values while enabled.
task.spawn(function()
    while true do
        task.wait(0.5)
        if BypassEnabled then
