...
local FallProtectionConnection = nil

local function InitializeFallProtection()
    if FallProtectionConnection then return end

    FallProtectionConnection = RS.Heartbeat:Connect(function()
        ...
        if not HumanoidNow then return end
        ...
        if VelocityY < -50 then
            ...
            RootPart.AssemblyLinearVelocity = Vector3.new(Velocity.X, -10, Velocity.Z)
        end
    end)
end

InitializeFallProtection()
...
IceTweenButton.MouseButton1Click:Connect(function()
    ...
    -- Streaming-safe fixed paths only. Try both:
    ...
    local Shard = Chat and Chat:FindFirstChild("ActiveIceShard")
...
local function GetIceShard()
    ...
    local Shard = Chat:FindFirstChild("ActiveIceShard")
...
task.defer(function()
    local ok, err = pcall(function()
        ...
        local function usePrompt(name)
            ...
            if not prompt then
                warn("[VENDETTA ACQUISITION] No prompt found for " .. name)
                return false
            end
            ...
            -- Fallback for environments where fireproximityprompt is unavailable.
            pcall(function()
        ...
        local function openSelector()
            ...
            selector.Position = UDim2.new(.5, -135, .5, -205)
