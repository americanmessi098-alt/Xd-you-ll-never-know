--// VENDETTA HUB v17 - NON-INVASIVE DIAGNOSTIC
--// Based directly on v12. The original code is not wrapped or reordered.
print("[VENDETTA DIAGNOSTIC] Script execution reached Vendetta source.")

local __VD_LogOK = pcall(function()
    local __VD_LogService = game:GetService("LogService")
    __VD_LogService.MessageOut:Connect(function(__VD_Message, __VD_MessageType)
        if __VD_MessageType == Enum.MessageType.MessageError then
            warn("[VENDETTA DIAGNOSTIC] Roblox reported: " .. tostring(__VD_Message))
            local __VD_Line = string.match(tostring(__VD_Message), "[Ll]ine%s+(%d+)")
            if __VD_Line then
                warn("[VENDETTA DIAGNOSTIC] Reported error line: " .. __VD_Line)
            end
        end
    end)
end)

if not __VD_LogOK then
    warn("[VENDETTA DIAGNOSTIC] LogService listener could not be installed; continuing normally.")
end

print("[VENDETTA DIAGNOSTIC] Beginning original v12 code...")
...
print("[VENDETTA DIAGNOSTIC] Vendetta Hub reached the end of the original script.")
