--// VENDETTA STARTUP DIAGNOSTICS
--// This listener reports runtime errors (including the Luau line number) in the executor console.
local __VendettaLogService = game:GetService("LogService")
local __VendettaStartupDone = false
local __VendettaStartupStarted = os.clock()

local __VendettaErrorConnection
__VendettaErrorConnection = __VendettaLogService.MessageOut:Connect(function(Message, MessageType)
    if MessageType == Enum.MessageType.MessageError then
        warn("[VENDETTA DIAGNOSTIC] SCRIPT ERROR: " .. tostring(Message))
        warn("[VENDETTA DIAGNOSTIC] Look for 'Script:LINE' or '[string ...]:LINE' in the error above.")
    end
end)

print("[VENDETTA DIAGNOSTIC] Startup began.")
...
--// Inventory desync is intentionally limited to 3 seconds per click.
--==================================================
...
local function StartInventoryDesync()
    ...
    AcquisitionDesyncConnection = RS.Heartbeat:Connect(function()
        if not AcquisitionDesyncRunning then
            return
        end
        ...
        local Tools = GetAllToolsForDesync()
        if #Tools == 0 then
            return
        end
    ...
    -- Hard 3-second lifetime for every acquisition click.
    task.delay(3, function()
        if AcquisitionDesyncRunning then
            StopInventoryDesync()
        end
    end)
end

local function FindNamedInstance(Name)
...
local function FindPromptIn(Object)
    if not Object then
        return nil
    end
...
local function TriggerPromptFor(Name)
    local Target = FindNamedInstance(Name)
    if not Target then
        Notify("Vendetta Hub", Name .. " not found.", 3)
        return false
    end

    local Prompt = FindPromptIn(Target)
    if not Prompt then
        Notify("Vendetta Hub", "No ProximityPrompt found for " .. Name .. ".", 3)
        return false
    end
    ...
    pcall(function()
        ...
        elseif Prompt.Enabled then
            -- Fallback for environments exposing the prompt interaction API.
...
local function RunBreathingAcquisition(TrainerName)
    local TargetPosition = TrainerPoints[TrainerName]
    if not TargetPosition then
        return
    end

    if BreathingSelector then
        BreathingSelector:Destroy()
        BreathingSelector = nil
    end

    task.spawn(function()
        TweenAndWait(KatanaSellerPosition)
        task.wait(0.1)
        TriggerPromptFor("Katana Seller")
        task.wait(1)
        TweenAndWait(TargetPosition)
...
local function OpenBreathingSelector()
    if BreathingSelector then
        BreathingSelector.Visible = true
        return
    end

    local Selector = Instance.new("Frame")
    Selector.Name = "BreathingTrainerSelector"
    Selector.Size = UDim2.fromOffset(270, 410)
    Selector.Position = UDim2.new(0.5, -135, 0.5, -205)
    ...
    Close.BackgroundColor3 = Color3.fromRGB(28, 29, 38)
    ...
    Scroll.Position = UDim2.fromOffset(10, 54)
    Scroll.BackgroundTransparency = 1
    Scroll.BorderSizePixel = 0
    Scroll.ScrollBarThickness = 4
    Scroll.CanvasSize = UDim2.fromOffset(0, #TrainerOrder * 48 + 8)
    Scroll.ZIndex = 51
    ...
    for Index, TrainerName in ipairs(TrainerOrder) do
        local TrainerButton = Instance.new("TextButton", Scroll)
        TrainerButton.Size = UDim2.new(1, -4, 0, 41)
        TrainerButton.BackgroundColor3 = Color3.fromRGB(19, 20, 27)
        TrainerButton.BorderSizePixel = 0
        TrainerButton.Text = TrainerName
        TrainerButton.TextColor3 = Color3.new(1, 1, 1)
        TrainerButton.Font = Enum.Font.GothamMedium
    ...
    BreathingSelector = Selector
end

AutoBreathingsButton.MouseButton1Click:Connect(function()
    OpenBreathingSelector()
end)

AutoBDAsButton.MouseButton1Click:Connect(function()
    task.spawn(function()
        local OldAkuma = FindNamedInstance("Old Akuma")
        local OldAkumaPosition = GetInstancePosition(OldAkuma)

        if not OldAkumaPosition then
            Notify("Vendetta Hub", "Old Akuma not found.", 3)
            return
        end

        TweenAndWait(OldAkumaPosition)
        TriggerPromptFor("Old Akuma")

        StartInventoryDesync()
        task.wait(2.5)

        -- The second interaction happens before the 3-second desync lifetime ends.
        TriggerPromptFor("Old Akuma")
    end)
end)
...
__VendettaStartupDone = true
print("[VENDETTA DIAGNOSTIC] Startup completed successfully.")
print("[VENDETTA] Premium Hub loaded.")

-- Keep the error listener alive briefly after startup so delayed initialization errors are reported.
task.delay(15, function()
    if __VendettaErrorConnection then
        __VendettaErrorConnection:Disconnect()
        __VendettaErrorConnection = nil
    end
end)
