...
local function FindEarrings()
    ...
    -- Same core logic as the supplied Pastefy finder: inspect map's direct
    -- children for a Model named Barrel, rather than scanning the whole map.
    for _,Object in ipairs(Map:GetChildren()) do
        if Object:IsA("Model") and Object.Name == "Barrel" then
            local Part = Object:FindFirstChild("Part", true)
            if Part and Part:IsA("BasePart") then
                return Part
            end
            local Base = Object:FindFirstChildWhichIsA("BasePart", true)
