...
local IceESPButton = Button(Visuals,"Ice Shard ESP   •   OFF",184)
...
local IceTweenButton = Button(Items,"Tween → Active Ice Shard",184)
...
local HitboxButton = Button(Combat,"Hitbox ESP   •   OFF",96)
local BreathingButton = Button(Combat,"Auto Breathing   •   OFF",140)
local KatanaButton = Button(Combat,"Katana Hide   •   OFF",184)
local BlockBreakButton = Button(Combat,"Block Break Macro   •   OFF",228)
local StrongAttackButton = Button(Combat,"Auto Strong Attack   •   OFF",272)
local ComboLeaverButton = Button(Combat,"Combo Leaver",316)

local FriendTitle = Label(Combat,"MANUAL FRIEND IGNORE",360)
local FriendBox = Instance.new("TextBox",Combat)
FriendBox.Size = UDim2.new(1,-116,0,38)
FriendBox.Position = UDim2.fromOffset(6,394)
...
local AddFriendButton = Button(Combat,"ADD",400)
AddFriendButton.Size = UDim2.fromOffset(48,38)
AddFriendButton.Position = UDim2.new(1,-104,0,394)
local RemoveFriendButton = Button(Combat,"REMOVE",452)
RemoveFriendButton.Size = UDim2.fromOffset(98,38)
RemoveFriendButton.Position = UDim2.fromOffset(6,440)

local FriendListLabel = Label(Combat,"Ignored: none",494)
...
local NPCFarmButton = Button(FarmPage,"AutoFarm NPC   •   OFF",300)
local AutoGourdsButton = Button(FarmPage,"Auto Gourds   •   OFF",344)
local RedeemCodesButton = Button(FarmPage,"Redeem All Codes",388)
...
local function FindFlute()
    local ChatNPCs = workspace:FindFirstChild("chatnpcs", true)
    if not ChatNPCs then return nil end

    local Flute = ChatNPCs:FindFirstChild("Flute", true)
    if not Flute then return nil end
    if Flute:IsA("BasePart") then return Flute end
    return Flute:FindFirstChildWhichIsA("BasePart", true)
end
...
local function FindEarrings()
    ...
    -- The earring is the Barrel model itself. Match the original finder:
    -- only inspect direct children of workspace.map and use the model pivot.
...
IceTweenButton.MouseButton1Click:Connect(function()
    local Part = GetIceShardPart(GetIceShard())
    if not Part then
        Notify("Vendetta Hub", "Ice Shard not found.", 3)
        return
    end
    if IceShardTween then IceShardTween:Cancel() end
    local Distance = (Part.Position - HRP.Position).Magnitude
    local Duration = Distance <= 500 and 3 or 6
    IceShardTween = TS:Create(HRP,TweenInfo.new(Duration,Enum.EasingStyle.Quad,Enum.EasingDirection.InOut),{CFrame=Part.CFrame*CFrame.new(0,3,0)})
...
local function GetIceShardPart(Shard)
    ...
    for _,Object in ipairs(Shard:GetDescendants()) do
        if Object:IsA("BasePart") then
            local S = Object.Size
            local V = S.X*S.Y*S.Z
            if V > Volume then Volume,Largest = V,Object end
        end
    end
    return Largest
end
...
local function TweenTo(Target)
    ...
    -- Match the earring finder behavior: stop slightly above the Barrel.
    local Goal = TargetCFrame * CFrame.new(0,5,0)
...
EarringsTween.MouseButton1Click:Connect(function()
    local Part = FindEarrings()
    if not Part then
        Notify("Vendetta Hub", "Earrings not found.", 3)
        return
    end
    TweenTo(Part)
end)
